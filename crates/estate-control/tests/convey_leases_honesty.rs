//! `estate convey leases` prints the same Agents, hop-cite, and Authority
//! stack as status, doctor, reconcile, leases, and audit export, before the
//! hop lease JSON. Mismatch is `FAIL`. Deny and deny-default are `note`. A
//! match is quiet. None of those cites fail the command. A placement-actual
//! SKU still refuses before the hop lease JSON.
...
fn scratch() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("cell-convey-leases-honesty-{nanos}"));
...
fn convey_leases(estate: &Path, state: &Path) -> (bool, String, String) {
    let estate_s = estate.display().to_string();
    let state_s = state.display().to_string();
    run(&[
        "convey",
...
fn assert_stack_before_hop_json(stdout: &str, agents: &str, section: &str) {
    ...
    let json_at = stdout
        .find("\"hop_id\"")
        .unwrap_or_else(|| panic!("missing hop lease JSON\n{stdout}"));
...
#[test]
fn convey_leases_prints_agents_cites_and_authority_before_the_hop_list() {
    let dir = scratch();
    ...
    let (ok, stdout, stderr) = convey_leases(&estate_path, &state);
    assert!(ok, "{stdout}\n{stderr}");
    ...
    assert_stack_before_hop_json(&stdout, &agents, &section);
    assert!(stdout.contains("\"hop_id\": \"cell-one-box\""), "{stdout}");
...
#[test]
fn hop_mismatch_cite_does_not_fail_convey_leases() {
...
fn missing_mesh_is_not_enforced_and_invents_no_cites() {
    ...
    let auth_at = stdout.find("Authority\n---------\n").unwrap();
    let empty_at = stdout
        .rfind("no hop leases under")
        .unwrap_or_else(|| panic!("missing empty hop-lease line\n{stdout}"));
    assert!(agents_at < auth_at && auth_at < empty_at, "{stdout}");
    assert!(stdout.contains(&section), "{stdout}");
    assert!(
        stdout.contains("does not show that a worker called the conveyor"),
        "{stdout}"
...
fn agent_unplaced_refuses_before_the_hop_list() {
    ...
    assert!(!stderr.contains("\"hop_id\""), "{stderr}");
...
fn placement_sku_prints_agents_and_cites_and_omits_authority() {
    ...
    let estate = estate_schema::load_estate(&estate_path).unwrap();
    let agents = estate_schema::describe_agents_section(&estate);
    let (ok, first, stderr) = convey_leases(&estate_path, &state);
    assert!(ok, "{first}\n{stderr}");
...
fn help_names_the_stack_and_locks_hold() {
    let (ok, stdout, stderr) = run(&["convey", "leases", "--help"]);
    assert!(ok, "{stdout}\n{stderr}");
    let agents = stdout
    ...
    assert!(
        language.contains("`estate convey leases` prints"),
        "{language}"
    );
    ...
    assert!(
        changelog.contains("estate convey leases prints the honesty stack"),
        "{changelog}"
    );
    assert!(
