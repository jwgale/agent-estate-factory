...
fn gate_90_tip_names_cell_one_through_pr_161() {
    ...
    assert!(
        head.contains("a857f05a256b214a2069de42e6918130eb080d66"),
        "GATE-90 header must name the PR #189 tip SHA: {head}"
    );
    assert!(
        head.contains("through PR #189"),
        "GATE-90 header must name tip through PR #189: {head}"
    );
    assert!(
        !head.contains("through PR #187 (`4df2c56d5ab622ea3843ade97e4a0dec9ea5b01a`)"),
        "GATE-90 header must not freeze tip at PR #187: {head}"
    );
    assert!(
        head.contains("tip honesty through PR #187 is PR #188")
            && head.contains("eb5ff41865d62db85355dbea7747ef4571941d43"),
        "GATE-90 header must name PR #188 tip honesty through PR #187: {head}"
    );
    assert!(
        head.contains("make purpose-build-journey")
            && head.contains("docs/OPERATOR-DAY.md")
            && head.contains("docs/NORTH-STAR.md")
            && head.contains("PR #189"),
        "GATE-90 header must name the operator-surface purpose-build entry: {head}"
    );
    assert!(
        head.contains("0e4223ea28a73c3931c7545f3040fd14ea8ae273"),
        "GATE-90 header must name the PR #191 tip SHA: {head}"
    );
    assert!(
        head.contains("through PR #191"),
        "GATE-90 header must name tip through PR #191: {head}"
    );
    assert!(
        !head.contains("through PR #189 (`a857f05a256b214a2069de42e6918130eb080d66`)"),
        "GATE-90 header must not freeze tip at PR #189: {head}"
    );
    assert!(
        head.contains("tip honesty through PR #189 is PR #190")
            && head.contains("ccf23829c8a62c070f98eff558d2f8fe2c1ccaaf"),
        "GATE-90 header must name PR #190 tip honesty through PR #189: {head}"
    );
    assert!(
        head.contains("When an SLM fits mid-software-build, or on demand, the same print-only entry is `make purpose-build-journey`")
            && head.contains("Those two targets are the parts.")
            && head.contains("PR #191"),
        "GATE-90 header must name the mid-software-build purpose-build entry: {head}"
    );
    assert!(
        head.contains("86b1ad5d344006e7489b24fe14fef7b8b16215f4"),
        "GATE-90 header must name the PR #193 tip SHA: {head}"
    );
    assert!(
        head.contains("through PR #193"),
        "GATE-90 header must name tip through PR #193: {head}"
    );
    assert!(
        !head.contains("through PR #191 (`0e4223ea28a73c3931c7545f3040fd14ea8ae273`)"),
        "GATE-90 header must not freeze tip at PR #191: {head}"
    );
    assert!(
        head.contains("tip honesty through PR #191 is PR #192")
            && head.contains("d260cb7145df00bec613bb0b34c52272cb97cd49"),
        "GATE-90 header must name PR #192 tip honesty through PR #191: {head}"
    );
    assert!(
        head.contains("estate help enrich")
            && head.contains("estate help train")
            && head.contains("The index Journey line names that same path.")
            && head.contains("does not inline those bodies")
            && head.contains("operator section 18")
            && head.contains("PR #193"),
        "GATE-90 header must name the PR #193 help mid-software-build framing: {head}"
    );
    assert!(
    ...
    let tip189 = changelog
        .split("## This slice — GATE-90 and Cell One tip honesty through PR #189")
        .nth(1)
        .expect("CHANGELOG missing the PR #189 tip-honesty slice")
        .split("## This slice —")
        .next()
        .unwrap();
    assert!(
        tip189.contains("a857f05a256b214a2069de42e6918130eb080d66"),
        "{tip189}"
    );
    assert!(
        tip189.contains("eb5ff41865d62db85355dbea7747ef4571941d43"),
        "{tip189}"
    );
    assert!(
        tip189.contains("4df2c56d5ab622ea3843ade97e4a0dec9ea5b01a"),
        "{tip189}"
    );
    assert!(tip189.contains("PR #188"), "{tip189}");
    assert!(tip189.contains("PR #189"), "{tip189}");
    assert!(tip189.contains("make purpose-build-journey"), "{tip189}");
    assert!(tip189.contains("make purpose-build-pick"), "{tip189}");
    assert!(tip189.contains("make purpose-build-checklist"), "{tip189}");
    assert!(tip189.contains("docs/OPERATOR-DAY.md"), "{tip189}");
    assert!(tip189.contains("docs/NORTH-STAR.md"), "{tip189}");
    assert!(tip189.contains("DeepSeek-R1-Distill chat print-only"), "{tip189}");
    assert!(tip189.contains("GLM-4 Chat print-only"), "{tip189}");
    assert!(tip189.contains("The picker does not run them."), "{tip189}");
    assert!(tip189.contains("only live uniqueness prove"), "{tip189}");
    assert!(tip189.contains("make lf-beachhead-prepare"), "{tip189}");
    assert!(tip189.contains("via PR #143"), "{tip189}");
    assert!(
        tip189.contains("does not invent a new live PASS"),
        "{tip189}"
    );
    assert!(tip189.contains("43770130 3391"), "{tip189}");
    assert!(tip189.contains("does not add Kimi"), "{tip189}");
    assert!(
        tip189.contains("READY_FOR_LIVE_TEST`: no") || tip189.contains("READY_FOR_LIVE_TEST: no"),
        "{tip189}"
    );
    assert!(
        !tip189.contains("READY_FOR_LIVE_TEST: yes")
            && !tip189.contains("READY_FOR_LIVE_TEST`: yes"),
        "{tip189}"
    );
    assert!(!tip189.to_ascii_lowercase().contains("kimi/"), "{tip189}");
    assert!(
        !tip189.contains("through PR #187 (`4df2c56d5ab622ea3843ade97e4a0dec9ea5b01a`)"),
        "tip slice must not freeze at the PR #187 tip SHA: {tip189}"
    );

    let tip191 = changelog
        .split("## This slice — GATE-90 and Cell One tip honesty through PR #191")
        .nth(1)
        .expect("CHANGELOG missing the PR #191 tip-honesty slice")
        .split("## This slice —")
        .next()
        .unwrap();
    assert!(
        tip191.contains("0e4223ea28a73c3931c7545f3040fd14ea8ae273"),
        "{tip191}"
    );
    assert!(
        tip191.contains("ccf23829c8a62c070f98eff558d2f8fe2c1ccaaf"),
        "{tip191}"
    );
    assert!(
        tip191.contains("a857f05a256b214a2069de42e6918130eb080d66"),
        "{tip191}"
    );
    assert!(tip191.contains("PR #190"), "{tip191}");
    assert!(tip191.contains("PR #191"), "{tip191}");
    assert!(tip191.contains("make purpose-build-journey"), "{tip191}");
    assert!(tip191.contains("make purpose-build-pick"), "{tip191}");
    assert!(tip191.contains("make purpose-build-checklist"), "{tip191}");
    assert!(tip191.contains("docs/OPERATOR-DAY.md"), "{tip191}");
    assert!(tip191.contains("docs/NORTH-STAR.md"), "{tip191}");
    assert!(
        tip191.contains("When an SLM fits mid-software-build, or on demand, the same print-only entry is `make purpose-build-journey`."),
        "{tip191}"
    );
    assert!(tip191.contains("Those two targets are the parts."), "{tip191}");
    assert!(tip191.contains("only live uniqueness prove"), "{tip191}");
    assert!(tip191.contains("make lf-beachhead-prepare"), "{tip191}");
    assert!(tip191.contains("via PR #143"), "{tip191}");
    assert!(
        tip191.contains("does not invent a new live PASS"),
        "{tip191}"
    );
    assert!(tip191.contains("43770130 3391"), "{tip191}");
    assert!(tip191.contains("does not add Kimi"), "{tip191}");
    assert!(
        tip191.contains("does not add DeepSeek or GLM journey code"),
        "{tip191}"
    );
    assert!(
        tip191.contains("READY_FOR_LIVE_TEST`: no") || tip191.contains("READY_FOR_LIVE_TEST: no"),
        "{tip191}"
    );
    assert!(
        !tip191.contains("READY_FOR_LIVE_TEST: yes")
            && !tip191.contains("READY_FOR_LIVE_TEST`: yes"),
        "{tip191}"
    );
    assert!(!tip191.to_ascii_lowercase().contains("kimi/"), "{tip191}");
    assert!(
        !tip191.contains("through PR #189 (`a857f05a256b214a2069de42e6918130eb080d66`)"),
        "tip slice must not freeze at the PR #189 tip SHA: {tip191}"
    );

    let tip193 = changelog
        .split("## This slice — GATE-90 and Cell One tip honesty through PR #193")
        .nth(1)
        .expect("CHANGELOG missing the PR #193 tip-honesty slice")
        .split("## This slice —")
        .next()
        .unwrap();
    assert!(
        tip193.contains("86b1ad5d344006e7489b24fe14fef7b8b16215f4"),
        "{tip193}"
    );
    assert!(
        tip193.contains("d260cb7145df00bec613bb0b34c52272cb97cd49"),
        "{tip193}"
    );
    assert!(
        tip193.contains("0e4223ea28a73c3931c7545f3040fd14ea8ae273"),
        "{tip193}"
    );
    assert!(tip193.contains("PR #192"), "{tip193}");
    assert!(tip193.contains("PR #193"), "{tip193}");
    assert!(tip193.contains("estate help enrich"), "{tip193}");
    assert!(tip193.contains("estate help train"), "{tip193}");
    assert!(tip193.contains("index Journey line"), "{tip193}");
    assert!(tip193.contains("operator section 18"), "{tip193}");
    assert!(tip193.contains("make purpose-build-journey"), "{tip193}");
    assert!(tip193.contains("make purpose-build-pick"), "{tip193}");
    assert!(tip193.contains("make purpose-build-checklist"), "{tip193}");
    assert!(tip193.contains("does not inline those bodies"), "{tip193}");
    assert!(tip193.contains("only live uniqueness prove"), "{tip193}");
    assert!(tip193.contains("make lf-beachhead-prepare"), "{tip193}");
    assert!(tip193.contains("via PR #143"), "{tip193}");
    assert!(tip193.contains("does not invent a new live PASS"), "{tip193}");
    assert!(tip193.contains("43770130 3391"), "{tip193}");
    assert!(tip193.contains("does not add Kimi"), "{tip193}");
    assert!(
        tip193.contains("READY_FOR_LIVE_TEST`: no") || tip193.contains("READY_FOR_LIVE_TEST: no"),
        "{tip193}"
    );
    assert!(
        !tip193.contains("READY_FOR_LIVE_TEST: yes")
            && !tip193.contains("READY_FOR_LIVE_TEST`: yes"),
        "{tip193}"
    );
    assert!(!tip193.to_ascii_lowercase().contains("kimi/"), "{tip193}");
    assert!(
        !tip193.contains("through PR #191 (`0e4223ea28a73c3931c7545f3040fd14ea8ae273`)"),
        "tip slice must not freeze at the PR #191 tip SHA: {tip193}"
    );
}
...
fn target_c_live_uniqueness_prove_stays_recorded() {
    ...
    assert!(
        !section.contains("Tip framing\nthrough PR #187"),
        "the uniqueness section must not freeze tip framing at PR #187"
    );
    assert!(
        !section.contains("Tip framing\nthrough PR #189"),
        "the uniqueness section must not freeze tip framing at PR #189"
    );
    assert!(
        !section.contains("Tip framing\nthrough PR #191"),
        "the uniqueness section must not freeze tip framing at PR #191"
    );
    assert!(
        section.contains("86b1ad5d344006e7489b24fe14fef7b8b16215f4"),
        "the uniqueness section must name the PR #193 tip SHA"
    );
    assert!(
        section.contains("Tip honesty through PR #191 is PR #192"),
        "the uniqueness section must name PR #192"
    );
    assert!(
        section.contains("d260cb7145df00bec613bb0b34c52272cb97cd49"),
        "the uniqueness section must name the PR #192 SHA"
    );
    assert!(
        section.contains("0e4223ea28a73c3931c7545f3040fd14ea8ae273"),
        "the uniqueness section must name the PR #191 tip SHA"
    );
    assert!(
        section.contains("Tip honesty through PR #189 is PR #190"),
        "the uniqueness section must name PR #190"
    );
    assert!(
        section.contains("ccf23829c8a62c070f98eff558d2f8fe2c1ccaaf"),
        "the uniqueness section must name the PR #190 SHA"
    );
    assert!(
        section.contains("a857f05a256b214a2069de42e6918130eb080d66"),
        "the uniqueness section must name the PR #189 tip SHA"
    );
    assert!(
        section.contains("Tip honesty through PR #187 is PR #188"),
        "the uniqueness section must name PR #188"
    );
    assert!(
        section.contains("eb5ff41865d62db85355dbea7747ef4571941d43"),
        "the uniqueness section must name the PR #188 SHA"
    );
    assert!(
...
fn glm4_chat_journey_stays_print_only_and_off_gates() {
    ...
    assert!(
        ...
        "GATE-90 header names tip through PR #193"
    );
    assert!(
        !gate_head.contains("through PR #191 (`0e4223ea28a73c3931c7545f3040fd14ea8ae273`)"),
        "GATE-90 header must not freeze tip at PR #191"
    );
    assert!(gate_head.contains("tip honesty through PR #191 is PR #192"));
    assert!(gate_head.contains("d260cb7145df00bec613bb0b34c52272cb97cd49"));
    assert!(
        !gate_head.contains("through PR #189 (`a857f05a256b214a2069de42e6918130eb080d66`)"),
        "GATE-90 header must not freeze tip at PR #189"
    );
    assert!(gate_head.contains("tip honesty through PR #187 is PR #188"));
    assert!(gate_head.contains("eb5ff41865d62db85355dbea7747ef4571941d43"));
    ...
    let status = std::fs::read_to_string(root.join("docs/CELL-ONE-STATUS.md")).unwrap();
    let status_head: String = status.lines().take(45).collect::<Vec<_>>().join("\n");
    assert!(status_head.contains("through PR #193"));
    assert!(status_head.contains("86b1ad5d344006e7489b24fe14fef7b8b16215f4"));
    assert!(status_head.contains("Tip honesty through PR #191 is PR #192"));
    assert!(status_head.contains("d260cb7145df00bec613bb0b34c52272cb97cd49"));
    assert!(status_head.contains("0e4223ea28a73c3931c7545f3040fd14ea8ae273"));
    assert!(status_head.contains("Tip honesty through PR #189 is PR #190"));
    assert!(status_head.contains("glm4-chat-journey"));
    assert!(status_head.contains("Tip honesty through PR #187 is PR #188"));
    assert!(status_head.contains("eb5ff41865d62db85355dbea7747ef4571941d43"));
    ...
    assert!(status.contains("| tip honesty through #189 |"));
    assert!(status.contains("PR #188 (`eb5ff41865d62db85355dbea7747ef4571941d43`)"));
    assert!(status.contains("PR #189 (`a857f05a256b214a2069de42e6918130eb080d66`)"));
    assert!(!status.contains("The tip header stays through PR #185"));
    assert!(!status.contains("The tip header stays through PR #187"));
...
fn purpose_build_operator_surfaces_name_the_journey() {
    ...
    assert!(
        gate_head.contains("through PR #193")
            && gate_head.contains("86b1ad5d344006e7489b24fe14fef7b8b16215f4"),
        "GATE-90 tip header names through PR #193: {gate_head}"
    );
    assert!(
        gate_head.contains("tip honesty through PR #191 is PR #192")
            && gate_head.contains("d260cb7145df00bec613bb0b34c52272cb97cd49"),
        "GATE-90 header must name PR #192: {gate_head}"
    );
    assert!(
        !gate_head.contains("through PR #191 (`0e4223ea28a73c3931c7545f3040fd14ea8ae273`)"),
        "GATE-90 tip header must not freeze at PR #191: {gate_head}"
    );
    assert!(
        gate_head.contains("tip honesty through PR #189 is PR #190")
            && gate_head.contains("ccf23829c8a62c070f98eff558d2f8fe2c1ccaaf"),
        "GATE-90 header must name PR #190: {gate_head}"
    );
    assert!(
        !gate_head.contains("through PR #189 (`a857f05a256b214a2069de42e6918130eb080d66`)"),
        "GATE-90 tip header must not freeze at PR #189: {gate_head}"
    );
    assert!(
        gate_head.contains("tip honesty through PR #187 is PR #188")
            && gate_head.contains("eb5ff41865d62db85355dbea7747ef4571941d43"),
        "GATE-90 header must name PR #188: {gate_head}"
    );
    assert!(
        !gate_head.contains("through PR #187 (`4df2c56d5ab622ea3843ade97e4a0dec9ea5b01a`)"),
        "GATE-90 tip header must not freeze at PR #187: {gate_head}"
    );
    let row = gate
        .lines()
        .find(|line| line.contains("| `make purpose-build-pick` |"))
    ...
    assert!(
        status_head.contains("through PR #193")
            && status_head.contains("86b1ad5d344006e7489b24fe14fef7b8b16215f4"),
        "Cell One tip header names through PR #193"
    );
    assert!(
        !status_head.contains("what is on `main` through PR #191"),
        "Cell One snapshot must not freeze at PR #191"
    );
    assert!(
        !status_head.contains("what is on `main` through PR #189"),
        "Cell One snapshot must not freeze at PR #189"
    );
    assert!(
        !status_head.contains("what is on `main` through PR #187"),
        "Cell One snapshot must not freeze at PR #187"
    );
...
fn purpose_build_surfaces_name_mid_software_build_entry() {
    ...
    assert!(
        gate_head.contains("through PR #193")
            && gate_head.contains("86b1ad5d344006e7489b24fe14fef7b8b16215f4"),
        "tip header names through PR #193"
    );
    assert!(
        gate_head.contains("tip honesty through PR #191 is PR #192")
            && gate_head.contains("does not inline those bodies")
            && gate_head.contains("PR #193"),
        "tip story must name PR #192 and the PR #193 help framing"
    );
    assert!(
        !gate_head.contains("through PR #191 (`0e4223ea28a73c3931c7545f3040fd14ea8ae273`)"),
        "tip header must not freeze at PR #191"
    );
    assert!(
        !gate_head.contains("through PR #189 (`a857f05a256b214a2069de42e6918130eb080d66`)"),
        "tip header must not freeze at PR #189"
    );
    let row = gate
        .lines()
        .find(|line| line.contains("| `make purpose-build-journey` |"))
    ...
    assert!(status.contains("| tip honesty through #191 |"));
    assert!(status.contains("| tip honesty through #193 |"));
    assert!(status.contains("PR #192 (`d260cb7145df00bec613bb0b34c52272cb97cd49`)"));
    assert!(status.contains("PR #190 (`ccf23829c8a62c070f98eff558d2f8fe2c1ccaaf`)"));
    assert!(status.contains("PR #191 (`0e4223ea28a73c3931c7545f3040fd14ea8ae273`)"));
...
fn help_names_mid_software_build_purpose_build_entry() {
    ...
    assert!(
        gate_head.contains("tip honesty through PR #191 is PR #192")
            && gate_head.contains("d260cb7145df00bec613bb0b34c52272cb97cd49"),
        "GATE-90 tip story names PR #192: {gate_head}"
    );
    assert!(
        gate_head.contains("does not inline those bodies") && gate_head.contains("PR #193"),
        "GATE-90 tip story names the PR #193 help framing: {gate_head}"
    );
    assert!(
        !gate_head.contains("through PR #191 (`0e4223ea28a73c3931c7545f3040fd14ea8ae273`)"),
        "GATE-90 tip header must not freeze at PR #191: {gate_head}"
    ...
    assert!(status.contains("Tip stays through PR #191 (`0e4223ea28a73c3931c7545f3040fd14ea8ae273`)."));
    ...
    let slice = changelog
        ...
        .nth(1)
        .expect("CHANGELOG missing the help mid-software-build slice")
        .split("## This slice —")
        .next()
        .unwrap();
    for needle in [
        ...
        "make purpose-build-pick",
        "make purpose-build-checklist",
        "does not inline those bodies",
        "does not train, fuse, convert, shell out to ollama, promote, or apply",
        "does not invent a live PASS",
        "only live uniqueness prove",
        "make uniqueness-prove-checklist",
        "does not move the GATE-90 or Cell One tip header",
        "through PR #191",
        "0e4223ea28a73c3931c7545f3040fd14ea8ae273",
        "43770130 3391",
        "does not add Kimi",
        "does not add DeepSeek or GLM journey code",
        "CELL_TRAIN_LIVE=1",
        ...
        assert!(slice.contains(needle), "help mid-software-build CHANGELOG slice missing {needle}");
    }
    assert!(!slice.contains("READY_FOR_LIVE_TEST: yes"));
    assert!(changelog.contains("## This slice — GATE-90 and Cell One tip honesty through PR #191"));
