...
fn gate_90_tip_names_cell_one_through_pr_161() {
    ...
    assert!(
        head.contains("through PR #171"),
        "GATE-90 header must name tip through PR #171: {head}"
    );
    assert!(
...
fn unsloth_doctor_status_honesty_stays_optional_and_keeps_the_tip() {
    ...
    assert!(
        head.contains("through PR #171"),
        "GATE-90 header names tip through PR #167: {head}"
    );
    assert!(
        !head.contains("through PR #165 (`7a1b3d54d37b38977c5570679d7f3922b9404d4a`)"),
        "GATE-90 header must not freeze tip at PR #165: {head}"
    ...
    let status_head: String = status.lines().take(35).collect::<Vec<_>>().join("\n");
    assert!(status_head.contains("through PR #171"), "{status_head}");
    assert!(
...
fn mlx_lm_doctor_status_honesty_stays_optional_and_keeps_the_tip() {
    ...
    assert!(
        head.contains("through PR #171"),
        "GATE-90 header stays through PR #171: {head}"
    );
    assert!(
        !head.contains("through PR #172"),
        "GATE-90 header must not move to PR #172 in this slice: {head}"
    );
    ...
    assert!(status_head.contains("through PR #171"), "{status_head}");
    assert!(
        !status_head.contains("what is on `main` through PR #172"),
        "{status_head}"
    );
...
fn train_next_prints_target_c_recipe_and_stays_off_smoke() {
    ...
    assert!(
        head.contains("through PR #171"),
        "GATE-90 header must keep tip through PR #167: {head}"
    );
    assert!(
        head.contains("6a43ff12a91295739c5a9c8a8f1dc9cc9c084466"),
        "GATE-90 header must keep the PR #157 standing-next SHA: {head}"
    ...
    let status_head: String = status.lines().take(35).collect::<Vec<_>>().join("\n");
    assert!(
        status_head.contains("through PR #171"),
...
fn tokenizer_restore_names_dereference_and_keeps_tip_framing() {
    ...
    assert!(
        gate_head.contains("through PR #171"),
...
fn target_c_live_uniqueness_prove_stays_recorded() {
    ...
    assert!(
        section.contains("651f31a4dd3a84dad270d40dbdd3892081b9380e"),
        "the uniqueness section must name the PR #171 tip SHA"
    );
    assert!(
    ...
    assert!(
        gate_head.contains("through PR #171"),
        "GATE-90 header must stay through PR #167: {gate_head}"
    );
    assert!(
        gate_head.contains("6a43ff12a91295739c5a9c8a8f1dc9cc9c084466"),
        "GATE-90 header must keep the PR #157 standing-next SHA: {gate_head}"
