//! `estate convey list` prints the same Agents, hop-cite, and Authority stack
//! as status, doctor, reconcile, leases, convey leases, audits, history, and
//! audit export, before the hop decl list. Mismatch is `FAIL`. Deny and
//! deny-default are `note`. A match is quiet. None of those cites fail the
//! command. A placement-actual SKU omits Authority. `list_hops` then still
//! refuses that SKU before the hop JSON.
...
fn scratch() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("cell-convey-list-honesty-{nanos}"));
...
fn no_sections(text: &str) -> bool {
    !text.contains("Agents\n------")
        ...
        && !text.contains("\"id\": \"cell-one-box\"")
        && !text.contains("no hops under")
...
fn convey_list(estate: &Path, state: &Path) -> (bool, String, String) {
    let estate_s = estate.display().to_string();
    let state_s = state.display().to_string();
    run(&[
        "convey",
...
fn assert_stack_before_list(stdout: &str, agents: &str, section: &str) {
    ...
    let list_at = stdout
        .find("\"id\": \"cell-one-box\"")
        .unwrap_or_else(|| panic!("missing hop decl list\n{stdout}"));
    ...
    assert!(
        stdout.contains("\"capability\": \"notes-append\""),
        "{stdout}"
    );
...
#[test]
fn convey_list_prints_agents_cites_and_authority_before_the_hop_list() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
    let mesh = granted_box("notes-append");
    let hop_json = serde_json::to_string_pretty(&mesh.hops).unwrap();
    ...
    let (ok, stdout, stderr) = convey_list(&estate_path, &state);
    assert!(ok, "{stdout}\n{stderr}");
    ...
    assert_stack_before_list(&stdout, &agents, &section);
    assert_eq!(
...
#[test]
fn hop_mismatch_cite_does_not_fail_convey_list() {
    ...
    let auth_at = stdout.find("Authority\n---------\n").unwrap();
    let list_at = stdout.find("\"id\": \"cell-one-box\"").unwrap();
    assert!(
        agents_at < cite_at && cite_at < auth_at && auth_at < list_at,
        "{stdout}"
    );
    assert!(stdout.ends_with(&format!("{hop_json}\n")), "{stdout}");
    assert!(no_enforced_status_token(&stdout), "{stdout}");
    assert!(!stdout.contains("live PASS"), "{stdout}");
    assert_eq!(snapshot(&state), before);
...
fn missing_mesh_is_not_enforced_and_invents_no_cites() {
    ...
    let list_at = stdout
        .find("no hops under")
        .unwrap_or_else(|| panic!("missing empty hop list\n{stdout}"));
    assert!(agents_at < auth_at && auth_at < list_at, "{stdout}");
    assert!(stdout.contains(&section), "{stdout}");
    assert!(
        stdout.contains("does not show that a worker called the conveyor"),
    ...
    assert!(stdout.ends_with(&format!("no hops under {}\n", state.display())));
...
#[test]
fn agent_unplaced_refuses_before_the_hop_list() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
...
#[test]
fn mesh_parse_and_mesh_host_class_refuse_before_the_hop_list() {
    let dir = scratch();
    let state = dir.join("state");
    std::fs::create_dir_all(&state).unwrap();
    let estate_path = repo_root().join("examples/estate.yaml");
    ...
    assert!(!stdout.contains("\"id\": \"cell-one-box\""), "{stdout}");
    assert!(!stdout.contains("no hops under"), "{stdout}");
    assert_eq!(snapshot(&state), before);
...
#[test]
fn placement_sku_prints_agents_and_cites_omits_authority_and_refuses_the_hop_list() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
    ...
    let estate = estate_schema::load_estate(&estate_path).unwrap();
    let agents = estate_schema::describe_agents_section(&estate);
    let (ok, first, stderr) = convey_list(&estate_path, &state);
    assert!(ok, "{first}\n{stderr}");
    ...
    assert!(first.contains("\"id\": \"cell-one-box\""), "{first}");
...
#[test]
fn missing_estate_refuses_before_the_hop_list() {
    let dir = scratch();
    let state = dir.join("state");
    std::fs::create_dir_all(&state).unwrap();
    let missing = dir.join("no-such-estate.yaml");
    let (ok, stdout, stderr) = convey_list(&missing, &state);
    assert!(!ok, "{stdout}\n{stderr}");
    assert!(stdout.trim().is_empty(), "{stdout}");
    assert!(no_sections(&stdout), "{stdout}");
    assert!(no_sections(&stderr), "{stderr}");
...
fn help_names_the_stack_and_locks_hold() {
    let (ok, stdout, stderr) = run(&["convey", "list", "--help"]);
    assert!(ok, "{stdout}\n{stderr}");
    let agents = stdout
    ...
    assert!(
        stdout.contains("hop list reader still refuses that SKU before the hop JSON"),
        "{stdout}"
    );
    assert!(stdout.contains("list_hops"), "{stdout}");
    ...
    let at = language
        ...
        .unwrap_or_else(|| panic!("language omits convey list\n{language}"));
    let mine = &language[at..];
    let end = mine
        .find("Whether a hop lease")
        .unwrap_or_else(|| panic!("language convey list sentence did not end\n{mine}"));
    let mine = &mine[..end];
    ...
    assert!(
        mine.contains("hop list reader still refuses that SKU before the hop JSON"),
        "{mine}"
    );
    assert!(mine.contains("list_hops"), "{mine}");
    ...
    let head = changelog
        .split("## This slice — estate history")
        .next()
        .unwrap();
    assert!(
        head.contains("estate convey list prints the honesty stack"),
        "{head}"
    );
    ...
    assert!(
        head.contains("hop list reader still refuses that SKU before the hop JSON"),
        "{head}"
    );
    assert!(head.contains("list_hops"), "{head}");
