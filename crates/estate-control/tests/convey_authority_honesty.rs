//! `estate convey authority` prints the same Agents, hop-cite, and Authority
//! stack as status, doctor, reconcile, leases, convey leases, convey list,
//! convey expire, convey sync, audits, history, and audit export. Mismatch
//! is `FAIL`. Deny and deny-default are `note`. A match is quiet. None of
//! those cites fail the command. A placement-actual SKU omits Authority and
//! the file check succeeds. There is no later reader.
...
fn scratch() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("cell-convey-authority-honesty-{nanos}"));
...
fn assert_stack(stdout: &str, agents: &str, section: &str) {
    ...
    assert!(!stdout.contains("\"id\": \"cell-one-box\""), "{stdout}");
    assert!(no_enforced_status_token(stdout), "{stdout}");
    assert!(!stdout.contains("live PASS"), "{stdout}");
    assert!(
        stdout.trim_end().ends_with(section.trim_end()),
        "Authority is the last section\n{stdout}"
    );
}

#[test]
fn convey_authority_prints_agents_cites_and_authority() {
    let dir = scratch();
    ...
    let (ok, stdout, stderr) = convey_authority(&estate_path, &state);
    assert!(ok, "{stdout}\n{stderr}");
    ...
    assert_stack(&stdout, &agents, &section);
...
#[test]
fn hop_mismatch_cite_does_not_fail_convey_authority() {
    ...
    let auth_at = stdout.find("Authority\n---------\n").unwrap();
    assert!(agents_at < cite_at && cite_at < auth_at, "{stdout}");
    assert!(stdout.trim_end().ends_with(
        conveyor_proxy::describe_authority_section(
            &conveyor_proxy::authority_report(&state, &estate).unwrap(),
            &state,
        )
        .trim_end()
    ));
...
#[test]
fn agent_unplaced_refuses_before_any_section() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
...
#[test]
fn mesh_parse_and_mesh_host_class_refuse_before_any_section() {
    let dir = scratch();
    let state = dir.join("state");
    std::fs::create_dir_all(&state).unwrap();
    let estate_path = repo_root().join("examples/estate.yaml");
...
#[test]
fn placement_sku_prints_agents_and_cites_omits_authority_and_succeeds() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
    ...
    let estate = estate_schema::load_estate(&estate_path).unwrap();
    let agents = estate_schema::describe_agents_section(&estate);
    let (ok, first, stderr) = convey_authority(&estate_path, &state);
    assert!(ok, "{first}\n{stderr}");
    ...
    assert!(ok, "{stdout}\n{stderr}");
    assert!(stderr.is_empty(), "{stderr}");
    assert_eq!(stdout, format!("{agents}{between}\n"));
...
#[test]
fn missing_or_unreadable_estate_refuses_before_any_section() {
    let dir = scratch();
    let state = dir.join("state");
    std::fs::create_dir_all(&state).unwrap();
    let missing = dir.join("no-such-estate.yaml");
    let (ok, stdout, stderr) = convey_authority(&missing, &state);
    assert!(!ok, "{stdout}\n{stderr}");
    assert!(stdout.trim().is_empty(), "{stdout}");
    assert!(no_sections(&stdout), "{stdout}");
    assert!(no_sections(&stderr), "{stderr}");
    ...
    let before = snapshot(&state);
    let (ok, stdout, stderr) = convey_authority(&bad, &state);
    assert!(!ok, "{stdout}\n{stderr}");
    assert!(stdout.trim().is_empty(), "{stdout}");
    assert!(no_sections(&stdout), "{stdout}");
    assert!(no_sections(&stderr), "{stderr}");
...
fn help_names_the_stack_and_locks_hold() {
    ...
    assert!(stdout.contains("There is no later reader"), "{stdout}");
    ...
    assert!(stdout.contains("without hop cites"), "{stdout}");
    assert!(stdout.contains("conveyor-proxy authority"), "{stdout}");
    ...
    let at = language
        .find("`estate convey authority` is a file check")
        .unwrap_or_else(|| panic!("language omits convey authority\n{language}"));
    let mine = &language[at..];
    let end = mine
        .find("`estate plan` prints")
        .unwrap_or_else(|| panic!("language convey authority sentence did not end\n{mine}"));
    let mine = &mine[..end];
    assert!(
        mine.contains("missing-mesh") || mine.contains("not-enforced"),
        "{mine}"
    );
    ...
    assert!(mine.contains("There is no later reader"), "{mine}");
    assert!(mine.contains("diverges"), "{mine}");
    assert!(mine.contains("without hop cites"), "{mine}");
    assert!(mine.contains("render_hop_coverage_cites"), "{mine}");
    assert!(
        mine.contains("conveyor-mesh.json") || mine.contains("hop cites"),
        "{mine}"
    );
    ...
    let head = changelog
        .split("## This slice — estate convey sync")
        .next()
        .unwrap();
    assert!(
        head.contains("estate convey authority prints the honesty stack"),
        "{head}"
    );
    ...
    assert!(head.contains("There is no later reader"), "{head}");
    assert!(head.contains("diverges"), "{head}");
    assert!(head.contains("without hop cites"), "{head}");
