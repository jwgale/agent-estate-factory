//! `estate history` prints the same Agents, hop-cite, and Authority stack as
//! status, doctor, reconcile, leases, convey leases, audits, and audit
//! export, before the lifecycle history list. Mismatch is `FAIL`. Deny and
//! deny-default are `note`. A match is quiet. None of those cites fail the
//! command. A placement-actual SKU omits Authority and the history body
//! still prints.
...
fn scratch() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("cell-history-honesty-{nanos}"));
...
fn no_sections(text: &str) -> bool {
    !text.contains("Agents\n------")
        ...
        && !text.contains("lifecycle history (")
        && !text.contains("no lifecycle.jsonl")
        && !text.contains("hash=")
...
fn history(estate: &Path, state: &Path) -> (bool, String, String) {
    let estate_s = estate.display().to_string();
    let state_s = state.display().to_string();
    run(&["history", "--estate", &estate_s, "--state-dir", &state_s])
}
...
fn assert_stack_before_list(stdout: &str, agents: &str, section: &str) {
    ...
    let list_at = stdout
        ...
        .unwrap_or_else(|| panic!("missing lifecycle history list\n{stdout}"));
...
fn seed_lifecycle(state: &Path) {
    std::fs::write(
        state.join("lifecycle.jsonl"),
        "{\"ts\":\"2026-01-01T00:00:00Z\",\"from\":null,\"to\":\"suspended\",\"action\":\"suspend\",\"desired_hash\":\"sha256:abc\",\"note\":\"seed\"}\n",
    )
    .unwrap();
}

#[test]
fn history_prints_agents_cites_and_authority_before_the_history_list() {
    let dir = scratch();
    ...
    let (ok, stdout, stderr) = history(&estate_path, &state);
    assert!(ok, "{stdout}\n{stderr}");
...
#[test]
fn hop_mismatch_cite_does_not_fail_history() {
    ...
    let auth_at = stdout.find("Authority\n---------\n").unwrap();
    let list_at = stdout.find("lifecycle history (").unwrap();
    assert!(
...
#[test]
fn agent_unplaced_refuses_before_the_history_list() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
    ...
    assert!(state.join("lifecycle.jsonl").is_file());
    let before = snapshot(&state);
    let (ok, stdout, stderr) = history(&estate_path, &state);
    assert!(!ok, "{stdout}\n{stderr}");
    ...
    assert!(!stdout.contains("lifecycle history ("), "{stdout}");
...
#[test]
fn mesh_parse_and_mesh_host_class_refuse_before_the_history_list() {
    let dir = scratch();
    let state = dir.join("state");
    std::fs::create_dir_all(&state).unwrap();
    let estate_path = repo_root().join("examples/estate.yaml");
...
#[test]
fn placement_sku_prints_agents_and_cites_omits_authority_and_prints_history() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
    ...
    let estate = estate_schema::load_estate(&estate_path).unwrap();
    let agents = estate_schema::describe_agents_section(&estate);
    let (ok, first, stderr) = history(&estate_path, &state);
    assert!(ok, "{first}\n{stderr}");
    ...
    let list_at = first.find("lifecycle history (").unwrap();
...
fn help_names_the_stack_and_locks_hold() {
    let (ok, stdout, stderr) = run(&["history", "--help"]);
    assert!(ok, "{stdout}\n{stderr}");
    let agents = stdout
    ...
    assert!(stdout.contains("history body still prints"), "{stdout}");
    ...
    assert!(language.contains("`estate history` prints"), "{language}");
    assert!(language.contains("missing-mesh"), "{language}");
    assert!(
        language.contains("the history body still prints"),
        "{language}"
    );
    assert!(
    ...
    assert!(
        changelog.contains("estate history prints the honesty stack"),
        "{changelog}"
    );
    assert!(
    ...
    assert!(
        changelog.contains("the history body still prints"),
        "{changelog}"
    );
    assert!(
