//! `estate audits` prints the same Agents, hop-cite, and Authority stack as
//! status, doctor, reconcile, leases, convey leases, and audit export, before
//! the apply-audit list. Mismatch is `FAIL`. Deny and deny-default are
//! `note`. A match is quiet. None of those cites fail the command. A
//! placement-actual SKU omits Authority and still prints the audit list.
...
fn scratch() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("cell-audits-honesty-{nanos}"));
...
fn audits(estate: &Path, state: &Path) -> (bool, String, String) {
    let estate_s = estate.display().to_string();
    let state_s = state.display().to_string();
    run(&["audits", "--estate", &estate_s, "--state-dir", &state_s])
}
...
fn assert_stack_before_list(stdout: &str, agents: &str, section: &str) {
    ...
    let list_at = stdout
        .find("apply audits (")
        .unwrap_or_else(|| panic!("missing apply-audit list\n{stdout}"));
    ...
    assert!(stdout.contains("require_plan="), "{stdout}");
    assert!(stdout.contains("spawned_cloud=false"), "{stdout}");
    assert!(!stdout.contains("spawned_cloud=true"), "{stdout}");
    assert!(no_enforced_status_token(stdout), "{stdout}");
    assert!(!stdout.contains("live PASS"), "{stdout}");
}

fn seed_apply_audit(state: &Path) {
    std::fs::write(
        state.join("apply-audit.jsonl"),
        "{\"created_at\":\"2026-01-01T00:00:00Z\",\"desired_hash\":\"sha256:abc\",\"sessions\":1,\"imported_packs\":[],\"require_plan\":true,\"note\":\"seed\",\"covering_plan\":\"plan-seed\",\"cloud_agent_spawned\":false}\n",
    )
    .unwrap();
}

#[test]
fn audits_prints_agents_cites_and_authority_before_the_audit_list() {
    let dir = scratch();
    ...
    let (ok, stdout, stderr) = audits(&estate_path, &state);
    assert!(ok, "{stdout}\n{stderr}");
...
#[test]
fn hop_mismatch_cite_does_not_fail_audits() {
    ...
    let auth_at = stdout.find("Authority\n---------\n").unwrap();
    let list_at = stdout.find("apply audits (").unwrap();
    assert!(
...
#[test]
fn agent_unplaced_refuses_before_the_audit_list() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
    ...
    assert!(!stdout.contains("apply audits ("), "{stdout}");
    assert!(!stderr.contains("require_plan="), "{stderr}");
...
#[test]
fn mesh_parse_and_mesh_host_class_refuse_before_the_audit_list() {
    let dir = scratch();
    let state = dir.join("state");
    std::fs::create_dir_all(&state).unwrap();
    let estate_path = repo_root().join("examples/estate.yaml");
...
#[test]
fn placement_sku_prints_agents_and_cites_omits_authority_and_lists_audits() {
    let dir = scratch();
    let state = dir.join("state");
    let estate_path = repo_root().join("examples/estate.yaml");
    apply(&estate_path, &state, &dir);
    ...
    let estate = estate_schema::load_estate(&estate_path).unwrap();
    let agents = estate_schema::describe_agents_section(&estate);
    let (ok, first, stderr) = audits(&estate_path, &state);
    assert!(ok, "{first}\n{stderr}");
    ...
    let list_at = first.find("apply audits (").unwrap();
    let list = &first[list_at..];
    assert!(list.contains("require_plan="), "{list}");
...
fn help_names_the_stack_and_locks_hold() {
    let (ok, stdout, stderr) = run(&["audits", "--help"]);
    assert!(ok, "{stdout}\n{stderr}");
    let agents = stdout
    ...
    assert!(language.contains("`estate audits` prints"), "{language}");
    ...
    assert!(
        changelog.contains("estate audits prints the honesty stack"),
        "{changelog}"
    );
    assert!(
