...
pub(crate) fn cmd_enrich_local_seat(
    prepared_dir: &Path,
    weights: Option<&Path>,
    adapter: Option<&Path>,
    runtime: &str,
...
pub(crate) fn cmd_enrich_merge_adapt(prepared_dir: &Path, adapter: &Path) -> Result<()> {
    let plan = plan_merge_adapt(prepared_dir, adapter)?;
    print!("{}", plan.report);
    if !plan.report.ends_with('\n') {
        println!();
    }
...
pub(crate) fn cmd_enrich_gguf_convert(prepared_dir: &Path, weights: &Path) -> Result<()> {
    let plan = plan_gguf_convert(prepared_dir, weights)?;
    print!("{}", plan.report);
    if !plan.report.ends_with('\n') {
        println!();
    }
