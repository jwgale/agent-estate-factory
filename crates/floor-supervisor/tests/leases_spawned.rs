//! `floor leases` does not print placement JSON when a cloud-agent
...
#[test]
fn floor_leases_does_not_print_a_spawned_cloud_lease() {
    let state = tmp("floor");
    let leases = ["leases", "--state-dir", &state.display().to_string()];

    let (ok, text) = run(&leases);
    assert!(ok, "{text}");
