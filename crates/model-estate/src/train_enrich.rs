...
pub(crate) const UNSLOTH_GUIDE_DOC: &str =
...
fn next_markdown(
    ...
    let (handoff, import_line, path_note) = if driver_id == "ollama-modelfile" {
        ...
        let reproduce = llamafactory_reproduce_notes(method, train_base);
        (
            format!(
...
/// and in `examples/train_qlora/qwen3_lora_sft_otfq.yaml`.
/// `constants.py` registers that id, `Qwen3-30B-A3B-Instruct-2507`,
/// `Qwen3-235B-A22B-Instruct-2507`, and `Qwen3-Next-80B-A3B-Instruct`
/// with `template="qwen3_nothink"`. Thinking and base checkpoints in that
/// file use `template="qwen3"`. The hub id `Qwen/Qwen3-4B` is the thinking
/// checkpoint (display name `Qwen3-4B-Thinking`). Qwen2 and Qwen2.5 text use
/// `template="qwen"`. `template.py` registers `qwen`, `qwen3`
/// (`ReasoningTemplate`), and `qwen3_nothink`.
/// A name that contains `thinking` stays on `qwen3`. A name that contains `nothink` uses `qwen3_nothink`.
/// This hint is wider than the reproduce notes. Those notes are only the
/// official `qwen3_nothink` text ids. The QLoRA note is on `llamafactory-qlora`.
/// The LoRA note is on `llamafactory-lora`.
/// Qwen2.5 Instruct text ids stay template `qwen`. Their QLoRA note is separate.
...
fn template_for_segment(segment: &str) -> Option<&'static str> {
    ...
    if name.contains("qwen3") {
        ...
        Some(template)
    } else if let Some(template) = glm_template_for_segment(&name) {
        Some(template)
    } else {
...
/// LLaMA-Factory `register_model_group` templates for GLM.
/// Longer stems win. `glm-4` is a prefix of `glm-4.1v`, `glm-4.5`, `glm-4.5v`,
/// `glm-4.6v`, and `glm-4.7`, and `.` is not an alphanumeric boundary, so a
/// short `glm-4` stem would label those groups `glm4`. `constants.py` names
/// the GLM-4-9B / 0414 text group `glm4`. `template.py` registers `glm4`.
/// There is no `glm_4` name. ChatGLM3 is `chatglm3`. GLM-Z1 is `glmz1`.
/// GLM-4.1V is `glm4v`. GLM-4.5 is `glm4_moe`. GLM-4.5V and GLM-4.6V are
/// `glm4_5v`. GLM-4.7 is `glm4_7`. GLM-OCR is `glm_ocr`.
/// HF cache directories keep the repo id in one segment
/// (`models--zai-org--glm-4-9b-chat`).
fn glm_template_for_segment(name: &str) -> Option<&'static str> {
    const GROUPS: &[(&[&str], &str)] = &[
        (&["chatglm3", "chatglm-3"], "chatglm3"),
        (&["glm-ocr", "glm_ocr"], "glm_ocr"),
        (&["glm-z1", "glmz1"], "glmz1"),
        (&["glm-4.1v", "glm4.1v", "glm-4-1v", "glm4v"], "glm4v"),
        (
            &["glm-4.6v", "glm4.6v", "glm-4.5v", "glm4.5v", "glm4_5v"],
            "glm4_5v",
        ),
        (&["glm-4.7", "glm4.7", "glm4_7"], "glm4_7"),
        (&["glm-4.5", "glm4.5", "glm4_moe", "glm4-moe"], "glm4_moe"),
        (&["glm-4", "glm4"], "glm4"),
    ];
    for (stems, template) in GROUPS {
        if stems.iter().any(|stem| stem_at_boundary(name, stem)) {
...
/// LoRA handoff when the train base is Phi-3 mini, Phi-3 medium, or Phi-3.5.
/// Empty for Phi-3-small (`phi_small`), for Phi-4 (`phi4`), for Phi-4-mini
/// (`phi4_mini`), and for the QLoRA card.
...
fn phi3_lora_reproduce_note(method: LlamaFactoryMethod, train_base: &str) -> String {
    ...
    if segment_is_phi3_instruct(segment) {
        format!("{PHI3_LORA_REPRODUCE_NOTE}\n\n")
    } else {
        String::new()
    }
}
...
/// LoRA handoff when the train base is Llama-3.2 Instruct (1B or 3B).
/// Empty for vision (`mllama`), for other `llama3` families, and for the QLoRA card.
...
fn llama32_lora_reproduce_note(method: LlamaFactoryMethod, train_base: &str) -> String {
    ...
    if segment_is_llama32_text_instruct(segment) {
        format!("{LLAMA32_LORA_REPRODUCE_NOTE}\n\n")
    } else {
        String::new()
    }
}
...
/// LoRA handoff when the train base is Mistral-7B Instruct (v0.1, v0.2, or v0.3).
/// Empty for a Mistral-7B base, for Mixtral, for Mistral-Small, for Mistral-Nemo,
/// for LLaVA-NeXT-Mistral, and for the QLoRA card.
...
fn mistral_lora_reproduce_note(method: LlamaFactoryMethod, train_base: &str) -> String {
    ...
    if segment_is_mistral_instruct(segment) {
        format!("{MISTRAL_LORA_REPRODUCE_NOTE}\n\n")
    } else {
        String::new()
    }
}

/// QLoRA handoff when the train base is the LLaMA-Factory `qwen3_nothink` text group.
/// Empty for Qwen3 thinking and base (`qwen3`), for Qwen2 and Qwen2.5 (`qwen`),
/// for a broad instruct name that is not that group, and for the LoRA card.
/// Qwen2.5 Instruct text ids are `qwen25_instruct_qlora_reproduce_note`.
const QWEN3_INSTRUCT_QLORA_REPRODUCE_NOTE: &str = "Reproduce target beside Phi-3, Llama-3.2, Gemma-2, Mistral, and Qwen2.x LoRA/QLoRA. Qwen3 Instruct (Qwen/Qwen3-4B-Instruct-2507, Qwen/Qwen3-30B-A3B-Instruct-2507, Qwen/Qwen3-235B-A22B-Instruct-2507, and Qwen/Qwen3-Next-80B-A3B-Instruct) uses LLaMA-Factory template qwen3_nothink. examples/train_lora/qwen3_lora_sft.yaml and examples/train_qlora/qwen3_lora_sft_otfq.yaml use Qwen/Qwen3-4B-Instruct-2507 and that template. A Qwen3 thinking or base checkpoint (Qwen/Qwen3-4B, Qwen/Qwen3-4B-Thinking-2507, Qwen/Qwen3-4B-Base, and Qwen/Qwen3-Next-80B-A3B-Thinking) uses template qwen3 and is not this reproduce target. Qwen2 stays template qwen and is not this reproduce target. Qwen2.5 Instruct, including Qwen/Qwen2.5-0.5B-Instruct, stays template qwen and is a different reproduce target. Qwen3-VL Instruct and Qwen3-Omni Instruct are different LLaMA-Factory groups (qwen3_vl_nothink and qwen3_omni_nothink) and are not this reproduce target. The Phi-3 reproduce target stays template phi or phi_small. The Llama-3.2 Instruct reproduce target stays template llama3. The Gemma-2 Instruct reproduce target stays template gemma2. The Mistral Instruct reproduce target stays template mistral. This QLoRA recipe keeps quantization_method bnb and quantization_bit 4. The seat tag and the train base stay separate. This factory does not download weights.";

fn qwen3_instruct_qlora_reproduce_note(method: LlamaFactoryMethod, train_base: &str) -> String {
    if method != LlamaFactoryMethod::Qlora {
        return String::new();
    }
    let Some(segment) = llamafactory_template_segment(train_base) else {
    ...
    if segment_is_qwen3_instruct(segment) {
        format!("{QWEN3_INSTRUCT_QLORA_REPRODUCE_NOTE}\n\n")
    } else {
        String::new()
    }
}

/// QLoRA handoff when the train base is the LLaMA-Factory Qwen2.5 Instruct text group.
...
fn qwen25_instruct_qlora_reproduce_note(method: LlamaFactoryMethod, train_base: &str) -> String {
    ...
    if segment_is_qwen25_instruct(segment) {
        format!("{QWEN25_INSTRUCT_QLORA_REPRODUCE_NOTE}\n\n")
    } else {
        String::new()
    }
}
...
fn qwen3_instruct_lora_reproduce_note(method: LlamaFactoryMethod, train_base: &str) -> String {
    ...
    if segment_is_qwen3_instruct(segment) {
        format!("{QWEN3_INSTRUCT_LORA_REPRODUCE_NOTE}\n\n")
    } else {
        String::new()
    }
}

/// LoRA handoff when the train base is Gemma-2 Instruct (`-it` or `Instruct`).
/// Empty for a Gemma-2 base, for original Gemma, for Gemma-3, and for the QLoRA card.
/// This is the non-quant twin of `gemma2_qlora_reproduce_note`.
...
fn gemma2_lora_reproduce_note(method: LlamaFactoryMethod, train_base: &str) -> String {
    ...
    if segment_is_gemma2_instruct(segment) {
        format!("{GEMMA2_LORA_REPRODUCE_NOTE}\n\n")
    } else {
        String::new()
    }
}
...
/// QLoRA handoff when the train base is an official GLM-4 Chat id in the
...
/// for `glm-4-9b-chat-hf`, and for the LoRA card.
/// `constants.py` registers GLM-4-9B, GLM-4-9B-Chat, GLM-4-9B-1M-Chat,
/// GLM-4-0414-9B-Chat, GLM-4-0414-32B-Base, and GLM-4-0414-32B-Chat with
...
fn qlora_reproduce_notes(method: LlamaFactoryMethod, train_base: &str) -> String {
    ...
    note.push_str(&glm4_chat_qlora_reproduce_note(method, train_base));
    note
}

fn llamafactory_reproduce_notes(method: LlamaFactoryMethod, train_base: &str) -> String {
    let mut note = qlora_reproduce_notes(method, train_base);
    note.push_str(&phi3_lora_reproduce_note(method, train_base));
...
/// Phi-3 mini, Phi-3 medium, and Phi-3.5. `phi_template_for_segment` returns
/// `phi` for that group, the same scan the QLoRA card uses for those ids.
/// Phi-3-small is `phi_small`. Phi-4 is `phi4`. Phi-4-mini is `phi4_mini`.
/// The QLoRA note also covers `phi_small`. This helper does not.
fn segment_is_phi3_instruct(segment: &str) -> bool {
    phi_template_for_segment(&segment.to_ascii_lowercase()) == Some("phi")
}
...
/// Official `template="qwen3_nothink"` text ids in `constants.py`.
/// `Qwen3-4B-Instruct-2507` is the smallest and the id in
/// `examples/train_lora/qwen3_lora_sft.yaml`. A thinking or base id uses
/// `qwen3`. Qwen2 and Qwen2.5 use `qwen`. Qwen3-VL and Qwen3-Omni Instruct
/// names also contain `qwen3` and `instruct`; they are not these stems.
fn segment_is_qwen3_instruct(segment: &str) -> bool {
    let name = segment.to_ascii_lowercase();
    if template_for_segment(segment) != Some("qwen3_nothink") {
        return false;
    }
    const STEMS: &[&str] = &[
...
/// Official Qwen2.5 Instruct text ids in the `template="qwen"` group in `constants.py`.
/// The same `register_model_group` also lists base, Coder, and Math ids.
/// Those are not this reproduce target. Qwen2 Instruct is the previous group
/// and uses the same template. Qwen2.5-VL is `qwen2_vl`. Qwen2.5-Omni is
/// `qwen2_omni`. GPTQ and AWQ checkpoints are already quantized. A name that
/// contains `thinking` is not this target. Qwen2.5 has no thinking id in that
/// group. `template.py` registers `qwen`. There is no `qwen2_5` name.
/// `Qwen2.5-7B-Instruct-1M` matches `qwen2.5-7b-instruct` because `-` is a boundary.
fn segment_is_qwen25_instruct(segment: &str) -> bool {
    let name = segment.to_ascii_lowercase();
    if template_for_segment(segment) != Some("qwen") {
        return false;
    }
...
/// Official GLM-4 Chat ids in the `template="glm4"` group in `constants.py`.
/// That same `register_model_group` also lists `GLM-4-9B` and
/// `GLM-4-0414-32B-Base`. Those bases use `glm4` and are not this reproduce
/// target. Hub ids: `glm-4-9b-chat`, `glm-4-9b-chat-1m`, `GLM-4-9B-0414`, and
/// `GLM-4-32B-0414`. Display names: `GLM-4-9B-Chat`, `GLM-4-9B-1M-Chat`,
/// `GLM-4-0414-9B-Chat`, and `GLM-4-0414-32B-Chat`. The DEFAULT DownloadSource
/// for GLM-4-9B-Chat is `zai-org/glm-4-9b-chat`. `glm-4-9b-chat-hf` is not in
/// that group. GLM-Z1 is `glmz1`. Vision ids are `glm4v` or `glm4_5v`.
/// GLM-4.5 is `glm4_moe`. GLM-4.7 is `glm4_7`. GLM-OCR is `glm_ocr`.
/// ChatGLM3 is `chatglm3`. GPTQ, AWQ, and GGUF checkpoints are already
/// converted or quantized. A `-` after a chat stem is a boundary, so
/// `glm-4-9b-chat-1m` matches `glm-4-9b-chat` and `glm-4-9b-chat-hf` matches
/// that stem too. The `hf` stem drops the transformers sibling.
fn segment_is_glm4_chat(segment: &str) -> bool {
    let name = segment.to_ascii_lowercase();
    if template_for_segment(segment) != Some("glm4") {
        return false;
    }
    if name.contains("gptq")
        || name.contains("awq")
        || name.contains("gguf")
        || name.contains("vision")
        || stem_at_boundary(&name, "hf")
        || stem_at_boundary(&name, "base")
    {
        return false;
    }
    const STEMS: &[&str] = &[
...
fn llamafactory_recipe_yaml(
    ...
    format!(
        "# schema: {schema}\n\
         ...
         # Qwen3 Instruct-2507 (Qwen3-4B-Instruct-2507, Qwen3-30B-A3B-Instruct-2507, Qwen3-235B-A22B-Instruct-2507) and Qwen3-Next-80B-A3B-Instruct are the qwen3_nothink reproduce target.\n\
         # examples/train_lora/qwen3_lora_sft.yaml uses Qwen/Qwen3-4B-Instruct-2507 and template qwen3_nothink.\n\
         # A Qwen3 thinking or base checkpoint uses qwen3 and is not that reproduce target.\n\
         # Qwen2 and Qwen2.5, including Qwen2.5-0.5B-Instruct, use qwen and are not that reproduce target.\n\
         # Qwen2.5 Instruct text ids (Qwen2.5-0.5B-Instruct, Qwen2.5-1.5B-Instruct, Qwen2.5-3B-Instruct, Qwen2.5-7B-Instruct, Qwen2.5-14B-Instruct, Qwen2.5-32B-Instruct, Qwen2.5-72B-Instruct, and the Instruct-1M ids) are the qwen reproduce target.\n\
         # A Qwen2.5 base, a name containing thinking, Qwen2.5-Coder, Qwen2.5-Math, Qwen2.5-VL, Qwen2-VL, Qwen2.5-Omni, and GPTQ or AWQ checkpoints are not that qwen reproduce target.\n\
         # Qwen2 Instruct uses qwen and is not that qwen reproduce target.\n\
         ...
         # Llama-3.2-1B-Instruct and Llama-3.2-3B-Instruct are the llama3 reproduce target.\n\
         # A Llama-3.2 base, Llama-3.1 Instruct, Llama-3.3 Instruct, Llama-3.2 vision, llama3-llava-next, and llama-30b are not that reproduce target.\n\
         ...
         # Gemma-2 Instruct (-it and the Instruct display name) is the gemma2 reproduce target.\n\
         # A Gemma-2 base uses gemma2 and is not that reproduce target.\n\
         ...
         # GLM-4 (GLM-4-9B, GLM-4-9B-Chat, GLM-4-9B-Chat-1M, GLM-4-9B-0414, GLM-4-32B-0414, and GLM-4-32B-Base-0414) uses glm4.\n\
         # GLM-4 Chat ids (GLM-4-9B-Chat, GLM-4-9B-Chat-1M, GLM-4-9B-0414, and GLM-4-32B-0414) are the glm4 reproduce target.\n\
         # GLM-4-9B and GLM-4-32B-Base-0414 use glm4 and are not that reproduce target.\n\
         # GLM-Z1 uses glmz1. GLM-4.1V uses glm4v. GLM-4.5 uses glm4_moe. GLM-4.5V and GLM-4.6V use glm4_5v. GLM-4.7 uses glm4_7. GLM-OCR uses glm_ocr. ChatGLM3 uses chatglm3.\n\
         # There is no glm_4 template. template.py registers glm4.\n\
...
mod tests {
    ...
    #[test]
    fn llamafactory_template_maps_qwen3_instruct_including_nested_paths() {
        let instruct = [
            ...
            "Qwen3-4B-Instruct-2507",
            "/opt/hf/Qwen/Qwen3-4B-Instruct-2507",
            "./weights/Qwen3-30B-A3B-Instruct-2507",
            "/opt/hf/Qwen/Qwen3-4B-Instruct-2507/weights",
            "/opt/hf/Qwen/Qwen3-235B-A22B-Instruct-2507/snapshots/deadbeef",
            "/home/user/.cache/huggingface/hub/models--Qwen--Qwen3-4B-Instruct-2507/snapshots/abc123",
            "/home/user/.cache/huggingface/hub/models--Qwen--Qwen3-30B-A3B-Instruct-2507/snapshots/def456",
            "/home/user/.cache/huggingface/hub/models--Qwen--Qwen3-235B-A22B-Instruct-2507/snapshots/ghi789",
            "/home/user/.cache/huggingface/hub/models--Qwen--Qwen3-Next-80B-A3B-Instruct/snapshots/jkl012",
            "/tmp/phi-4/Qwen3-4B-Instruct-2507",
            "/tmp/Qwen2.5-0.5B-Instruct/Qwen3-4B-Instruct-2507",
            "/tmp/Qwen3-4B-Thinking-2507/Qwen3-30B-A3B-Instruct-2507",
            "/tmp/gemma-2-2b-it/Qwen3-Next-80B-A3B-Instruct",
            "/tmp/Mistral-7B-Instruct-v0.3/Qwen3-4B-Instruct-2507",
            "/tmp/Llama-3.2-3B-Instruct/Qwen3-4B-Instruct-2507",
        ];
        for train in instruct {
            assert_eq!(llamafactory_template(train), "qwen3_nothink", "{train}");
            assert_ne!(llamafactory_template(train), "qwen3", "{train}");
            assert_ne!(llamafactory_template(train), "qwen", "{train}");
        }
        let thinking = [
            "Qwen/Qwen3-4B",
            "Qwen/Qwen3-0.6B",
            ...
            "Qwen/Qwen3-4B-Instruct-Thinking",
            "/opt/hf/Qwen/Qwen3-4B",
            "/opt/hf/Qwen/Qwen3-4B-Thinking-2507/weights",
            "/home/user/.cache/huggingface/hub/models--Qwen--Qwen3-4B-Thinking-2507/snapshots/abc123",
            "/tmp/Qwen3-4B-Instruct-2507/Qwen3-4B-Thinking-2507",
            "/tmp/models/Qwen3-4B/snapshots/deadbeef",
        ];
        for train in thinking {
            assert_eq!(llamafactory_template(train), "qwen3", "{train}");
            assert_ne!(llamafactory_template(train), "qwen3_nothink", "{train}");
            assert_ne!(llamafactory_template(train), "qwen", "{train}");
        }
        let qwen2 = [
            ...
            "/opt/hf/Qwen/Qwen2.5-0.5B-Instruct",
            "./weights/Qwen2.5-0.5B-Instruct",
            "/home/user/.cache/huggingface/hub/models--Qwen--Qwen2.5-0.5B-Instruct/snapshots/abc123",
            "/tmp/Qwen3-4B-Instruct-2507/Qwen2.5-0.5B-Instruct",
        ];
        for train in qwen2 {
            assert_eq!(llamafactory_template(train), "qwen", "{train}");
            assert_ne!(llamafactory_template(train), "qwen3_nothink", "{train}");
            assert_ne!(llamafactory_template(train), "qwen3", "{train}");
        }
        // constants.py uses qwen3_vl_nothink / qwen3_omni_nothink / qwen3_vl.
        // This scan still sees the qwen3 stem. It must not fall through to qwen.
        assert_ne!(llamafactory_template("Qwen/Qwen3-VL-4B-Instruct"), "qwen");
        assert_ne!(llamafactory_template("Qwen/Qwen3-VL-4B-Instruct"), "qwen3");
        assert_eq!(llamafactory_template("Qwen/Qwen3-VL-4B-Thinking"), "qwen3");
        assert_ne!(
            llamafactory_template("Qwen/Qwen3-Omni-30B-A3B-Instruct"),
            "qwen"
        );
        assert_eq!(
            llamafactory_template("Qwen/Qwen3-Omni-30B-A3B-Thinking"),
            "qwen3"
        );
        assert_ne!(llamafactory_template("Qwen/Qwen3.5-4B-Thinking"), "qwen");
        assert_ne!(
            llamafactory_template("Qwen/Qwen3.5-4B-Thinking"),
            "qwen3_nothink"
        );
        assert_eq!(
            llamafactory_template("/tmp/Qwen3-4B-Instruct-2507/Phi-3-mini-4k-instruct"),
            "phi"
        );
        assert_eq!(
            llamafactory_template("/tmp/Qwen3-4B-Instruct-2507/Llama-3.2-3B-Instruct"),
            "llama3"
        );
        assert_eq!(
            llamafactory_template("/tmp/Qwen3-4B-Instruct-2507/gemma-2-2b-it"),
            "gemma2"
        );
        assert_eq!(
            llamafactory_template("/tmp/Qwen3-4B-Instruct-2507/Mistral-7B-Instruct-v0.3"),
            "mistral"
        );
    }
    ...
    #[test]
    fn phi3_lora_prepare_emits_template_and_keeps_the_seat_split() {
        const PHI_LORA_NOTE: &str =
            "Reproduce target on the unquantized LoRA card, the non-quant twin of the Phi-3 Instruct QLoRA prepare.";
        const PHI_QLORA_NOTE: &str = "Reproduce target beside Qwen LoRA/QLoRA.";
        let root = tmp("phi3-instruct-lora");
        let pack_path = repo_root().join("examples/fixtures/phi3-instruct-lora.pack.json");
        let pack: PackManifest =
            serde_json::from_str(&std::fs::read_to_string(&pack_path).unwrap()).unwrap();
        assert_eq!(pack.id, "phi3-instruct-lora");
        assert_eq!(
            pack.train_base_model.as_deref(),
            Some("microsoft/Phi-3-mini-4k-instruct")
        );
        ...
        assert!(!doc.promoted && !doc.auto_apply && !doc.estate_rewritten);
        let recipe = std::fs::read_to_string(out.join("recipe.yaml")).unwrap();
        assert!(yaml_line(&recipe, "template: phi"), "{recipe}");
        assert!(!yaml_line(&recipe, "template: phi_small"), "{recipe}");
        assert!(!yaml_line(&recipe, "template: phi4"), "{recipe}");
        assert!(!yaml_line(&recipe, "template: phi4_mini"), "{recipe}");
        ...
        assert!(
            recipe.contains("Phi-3 mini, Phi-3 medium, and Phi-3.5 use phi. Phi-3-small uses phi_small."),
            "{recipe}"
        );
        assert!(
            recipe.contains("Phi-4 uses phi4. Phi-4-mini uses phi4_mini."),
            "{recipe}"
        );
        assert!(
            recipe.contains("model_name_or_path: \"microsoft/Phi-3-mini-4k-instruct\""),
        ...
        let export = std::fs::read_to_string(out.join("export.yaml")).unwrap();
        assert!(yaml_line(&export, "template: phi"), "{export}");
        assert!(
        ...
        let prepare_md = std::fs::read_to_string(out.join("PREPARE.md")).unwrap();
        for text in [&next, &prepare_md] {
            assert!(text.contains(PHI_LORA_NOTE), "{text}");
            assert!(
                text.contains("use LLaMA-Factory template phi."),
                "{text}"
            );
            assert!(text.contains("microsoft/Phi-3-mini-4k-instruct"), "{text}");
            assert!(
            ...
            assert!(text.contains("phi_small"), "{text}");
            assert!(text.contains("phi4_mini"), "{text}");
            assert!(!text.contains(PHI_QLORA_NOTE), "{text}");
            assert!(
        ...
        assert!(yaml_line(&qlora_recipe, "template: phi"), "{qlora_recipe}");
        ...
        assert!(qlora_next.contains(PHI_QLORA_NOTE), "{qlora_next}");
        assert!(qlora_prepare.contains(PHI_QLORA_NOTE), "{qlora_prepare}");
        assert!(!qlora_next.contains(PHI_LORA_NOTE), "{qlora_next}");
        assert!(!qlora_prepare.contains(PHI_LORA_NOTE), "{qlora_prepare}");
        ...
        for bad in ["llama3", "llama3:latest", "./llama3", "../llama3"] {
            ...
            assert!(!err.to_string().contains("microsoft/Phi"), "{bad}: {err}");
            assert!(!bad_out.exists(), "{bad}");
        }
        for bad in ["./phi-3-mini-4k-instruct", "/opt/hf/phi-3-mini-4k-instruct"] {
            let bad_estate = with_train_base(seated.clone(), bad);
        ...
        let nested_estate = with_train_base(
            seated.clone(),
            "./weights/microsoft/Phi-3.5-mini-instruct/weights",
        );
        let nested_out = root.join("nested-35");
        let nested = run(
            LLAMAFACTORY_LORA_ID,
            &bare,
            &nested_estate,
        ...
        let nested_train = nested.train_base_model.as_deref().unwrap();
        assert!(
            nested_train.ends_with("/weights/microsoft/Phi-3.5-mini-instruct/weights"),
            "{nested_train}"
        );
        assert_eq!(nested.base_model, "llama3");
        let nested_recipe = std::fs::read_to_string(nested_out.join("recipe.yaml")).unwrap();
        assert!(yaml_line(&nested_recipe, "template: phi"), "{nested_recipe}");
        assert!(
        ...
        let nested_next = std::fs::read_to_string(nested_out.join("NEXT.md")).unwrap();
        let nested_prepare = std::fs::read_to_string(nested_out.join("PREPARE.md")).unwrap();
        assert!(nested_next.contains(PHI_LORA_NOTE), "{nested_next}");
        assert!(nested_prepare.contains(PHI_LORA_NOTE), "{nested_prepare}");
        assert!(!nested_next.contains(PHI_QLORA_NOTE), "{nested_next}");

        let cases = [
            ("microsoft/Phi-3-mini-4k-instruct", "phi", true),
            ("microsoft/Phi-3-mini-128k-instruct", "phi", true),
            ("microsoft/Phi-3-medium-4k-instruct", "phi", true),
            ("microsoft/Phi-3-medium-128k-instruct", "phi", true),
            ("microsoft/Phi-3.5-mini-instruct", "phi", true),
            ("microsoft/Phi-3.5-MoE-instruct", "phi", true),
            ("Microsoft/PHI-3-MINI-4K-INSTRUCT", "phi", true),
            ("unsloth/Phi-3-mini-4k-instruct", "phi", true),
            ("/opt/hf/microsoft/Phi-3-mini-4k-instruct", "phi", true),
            (
                "/home/user/.cache/huggingface/hub/models--microsoft--Phi-3-mini-4k-instruct/snapshots/abc123def456",
                "phi",
                true,
            ),
            (
                "/home/user/.cache/huggingface/hub/models--microsoft--Phi-3.5-mini-instruct/snapshots/def456abc123",
                "phi",
                true,
            ),
            ("/tmp/phi-4/Phi-3-mini-4k-instruct", "phi", true),
            ("/tmp/qwen3-parent/Phi-3.5-MoE-instruct", "phi", true),
            ("microsoft/Phi-3-small-8k-instruct", "phi_small", false),
            ("microsoft/Phi-3-small-128k-instruct", "phi_small", false),
            ("/opt/hf/microsoft/Phi-3-small-8k-instruct", "phi_small", false),
            (
                "/home/user/.cache/huggingface/hub/models--microsoft--Phi-3-small-8k-instruct/snapshots/abc123def456",
                "phi_small",
                false,
            ),
            ("microsoft/phi-4", "phi4", false),
            ("microsoft/Phi-4-mini-instruct", "phi4_mini", false),
            ("/opt/hf/Phi-4-mini-instruct", "phi4_mini", false),
            ("/tmp/Phi-3-mini-4k-instruct/Phi-4", "phi4", false),
            ("microsoft/phi-2", "default", false),
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            if *template != "phi" {
                assert!(!yaml_line(&case_recipe, "template: phi"), "{train}\n{case_recipe}");
            }
            let case_export = std::fs::read_to_string(case_out.join("export.yaml")).unwrap();
            assert!(
                yaml_line(&case_export, &template_line),
                "{train}\n{case_export}"
            );
            ...
            assert_eq!(case_next.contains(PHI_LORA_NOTE), *note, "{train}\n{case_next}");
            assert_eq!(
                case_prepare.contains(PHI_LORA_NOTE),
                *note,
                "{train}\n{case_prepare}"
            );
            assert!(!case_next.contains(PHI_QLORA_NOTE), "{train}");
            ...
            let qlora_case_recipe = std::fs::read_to_string(qlora_case.join("recipe.yaml")).unwrap();
            assert!(
                !qlora_case_next.contains(PHI_LORA_NOTE),
                "{train}\n{qlora_case_next}"
            );
            assert!(
            ...
            let qlora_phi = matches!(*template, "phi" | "phi_small");
            assert_eq!(
                qlora_case_next.contains(PHI_QLORA_NOTE),
                qlora_phi,
                "{train}\n{qlora_case_next}"
            );
        }
        ...
        assert!(official_next.contains(PHI_LORA_NOTE), "{official_next}");
        assert!(!official_next.contains(PHI_QLORA_NOTE), "{official_next}");
        assert!(
        ...
        let sacred_base = with_train_base(seated.clone(), "cyera/Phi-3-mini-4k-instruct");
        ...
        let sku_base = with_train_base(
            seated.clone(),
            "/tmp/cell-one-hf/5090/Phi-3-mini-4k-instruct",
        );
    ...
    fn llama32_qlora_prepare_emits_template_bnb_and_keeps_the_seat_split() {
        ...
        assert!(
            lora_recipe.lines().any(|line| line.trim() == "lora_rank: 8"),
            "{lora_recipe}"
        );
        assert!(
            lora_recipe
                .lines()
                .any(|line| line.trim() == "packing: false"),
            "{lora_recipe}"
        );
        let lora_next = std::fs::read_to_string(lora_out.join("NEXT.md")).unwrap();
        ...
        assert!(
            !lora_prepare.contains("Reproduce target beside Phi-3 and Qwen LoRA/QLoRA."),
            "{lora_prepare}"
        );
    ...
    #[test]
    fn llama32_instruct_lora_prepare_emits_template_and_keeps_the_seat_split() {
        const LLAMA32_LORA_NOTE: &str = "Reproduce target on the unquantized LoRA card, the non-quant twin of the Llama-3.2 Instruct QLoRA prepare.";
        const LLAMA32_QLORA_NOTE: &str =
            "Reproduce target beside Phi-3 and Qwen LoRA/QLoRA.";
        const QWEN3_LORA_NOTE: &str = "Reproduce target on the unquantized LoRA card, the non-quant twin of the Qwen3 Instruct QLoRA prepare.";
        let root = tmp("llama32-instruct-lora");
        let pack_path = repo_root().join("examples/fixtures/llama32-instruct-lora.pack.json");
        let pack: PackManifest =
            serde_json::from_str(&std::fs::read_to_string(&pack_path).unwrap()).unwrap();
        assert_eq!(pack.id, "llama32-instruct-lora");
        assert_eq!(
            pack.train_base_model.as_deref(),
            Some("meta-llama/Llama-3.2-3B-Instruct")
        );
        ...
        assert!(!yaml_line(&recipe, "template: mllama"), "{recipe}");
        assert!(yaml_line(&recipe, "lora_rank: 8"), "{recipe}");
        assert!(yaml_line(&recipe, "lora_alpha: 16"), "{recipe}");
        assert!(yaml_line(&recipe, "packing: false"), "{recipe}");
        assert!(yaml_line(&recipe, "finetuning_type: lora"), "{recipe}");
        ...
        assert!(recipe.contains("does not run llamafactory-cli"), "{recipe}");
        let export = std::fs::read_to_string(out.join("export.yaml")).unwrap();
        assert!(yaml_line(&export, "template: llama3"), "{export}");
        assert!(
            !export.contains("quantization_bit") && !export.contains("quantization_method"),
            "{export}"
        );
        ...
        let prepare_md = std::fs::read_to_string(out.join("PREPARE.md")).unwrap();
        for text in [&next, &prepare_md] {
            assert!(text.contains(LLAMA32_LORA_NOTE), "{text}");
            assert!(
                text.contains("uses LLaMA-Factory template llama3."),
                "{text}"
            );
            assert!(text.contains("template mllama"), "{text}");
            assert!(text.contains("There is no llama3_2 template."), "{text}");
            assert!(text.contains("meta-llama/Llama-3.2-3B-Instruct"), "{text}");
            assert!(text.contains("Llama-3.2-1B-Instruct"), "{text}");
            assert!(
                text.contains("omits quantization_bit and quantization_method"),
                "{text}"
            );
            assert!(text.contains("lora_rank 8"), "{text}");
            assert!(text.contains("packing false"), "{text}");
            assert!(!text.contains(LLAMA32_QLORA_NOTE), "{text}");
        ...
        .unwrap();
        assert_eq!(qlora.base_model, "llama3");
        assert_eq!(qlora.seat_tag.as_deref(), Some("llama3"));
        assert_eq!(
            qlora.train_base_model.as_deref(),
            Some("meta-llama/Llama-3.2-3B-Instruct")
        );
        let qlora_recipe = std::fs::read_to_string(qlora_out.join("recipe.yaml")).unwrap();
        assert!(yaml_line(&qlora_recipe, "template: llama3"), "{qlora_recipe}");
        assert!(
        ...
        let qlora_next = std::fs::read_to_string(qlora_out.join("NEXT.md")).unwrap();
        let qlora_prepare = std::fs::read_to_string(qlora_out.join("PREPARE.md")).unwrap();
        assert!(qlora_next.contains(LLAMA32_QLORA_NOTE), "{qlora_next}");
        assert!(qlora_prepare.contains(LLAMA32_QLORA_NOTE), "{qlora_prepare}");
        assert!(!qlora_next.contains(LLAMA32_LORA_NOTE), "{qlora_next}");
        assert!(!qlora_prepare.contains(LLAMA32_LORA_NOTE), "{qlora_prepare}");
        ...
        for bad in ["./llama-3.2-3b-instruct", "/opt/hf/llama-3.2-3b-instruct"] {
            let bad_estate = with_train_base(seated.clone(), bad);
            let bad_out = root.join(format!("leaf-{}", bad.replace('/', "_")));
            let err = run(
                LLAMAFACTORY_LORA_ID,
        ...
        let nested_estate = with_train_base(
            seated.clone(),
            "./weights/meta-llama/Llama-3.2-1B-Instruct/weights",
        );
        let nested_out = root.join("nested-1b");
        let nested = run(
            LLAMAFACTORY_LORA_ID,
        ...
        let nested_train = nested.train_base_model.as_deref().unwrap();
        assert!(
            nested_train.ends_with("/weights/meta-llama/Llama-3.2-1B-Instruct/weights"),
            "{nested_train}"
        );
        assert_eq!(nested.base_model, "llama3");
        let nested_recipe = std::fs::read_to_string(nested_out.join("recipe.yaml")).unwrap();
        assert!(yaml_line(&nested_recipe, "template: llama3"), "{nested_recipe}");
        assert!(
            !nested_recipe.contains("quantization_bit")
                && !nested_recipe.contains("quantization_method"),
            "{nested_recipe}"
        ...
        let nested_next = std::fs::read_to_string(nested_out.join("NEXT.md")).unwrap();
        let nested_prepare = std::fs::read_to_string(nested_out.join("PREPARE.md")).unwrap();
        assert!(nested_next.contains(LLAMA32_LORA_NOTE), "{nested_next}");
        assert!(nested_prepare.contains(LLAMA32_LORA_NOTE), "{nested_prepare}");
        assert!(!nested_next.contains(LLAMA32_QLORA_NOTE), "{nested_next}");

        let cases = [
            ("meta-llama/Llama-3.2-1B-Instruct", "llama3", true),
            ("meta-llama/Llama-3.2-3B-Instruct", "llama3", true),
            ("META-LLAMA/LLAMA-3.2-1B-INSTRUCT", "llama3", true),
            ("unsloth/Llama-3.2-3B-Instruct", "llama3", true),
            (
                "/home/user/.cache/huggingface/hub/models--meta-llama--Llama-3.2-3B-Instruct/snapshots/abc123def456",
                "llama3",
                true,
            ),
            (
                "/home/user/.cache/huggingface/hub/models--meta-llama--Llama-3.2-1B-Instruct/snapshots/def456abc789",
                "llama3",
                true,
            ),
            ("/tmp/phi-4/Llama-3.2-1B-Instruct", "llama3", true),
            ("/tmp/qwen3-parent/Llama-3.2-3B-Instruct", "llama3", true),
            ("/tmp/llama-30b/Llama-3.2-3B-Instruct", "llama3", true),
            (
                "/opt/hf/meta-llama/Llama-3.2-3B-Instruct/weights",
                "llama3",
                true,
            ),
            ("meta-llama/Llama-3.2-3B", "llama3", false),
            ("meta-llama/Meta-Llama-3-8B-Instruct", "llama3", false),
            ("meta-llama/Meta-Llama-3.1-8B-Instruct", "llama3", false),
            ("meta-llama/Llama-3.3-70B-Instruct", "llama3", false),
            ("meta-llama/Llama-3.2-11B-Vision-Instruct", "mllama", false),
            ("meta-llama/Llama-3.2-90B-Vision-Instruct", "mllama", false),
            ("meta-llama/Llama-3.2-11B-Vision", "mllama", false),
            ("llava-hf/llama3-llava-next-8b-hf", "llava_next_llama3", false),
            ("huggyllama/llama-30b", "default", false),
            ("Qwen/Qwen2.5-0.5B-Instruct", "qwen", false),
            ("Qwen/Qwen3-4B-Instruct-2507", "qwen3_nothink", false),
            ...
            ("mistralai/Mistral-7B-Instruct-v0.3", "mistral", false),
            (
                "/tmp/Llama-3.2-3B-Instruct/Phi-3-mini-4k-instruct",
                "phi",
                false,
            ),
            (
                "/tmp/Llama-3.2-3B-Instruct/Qwen2.5-0.5B-Instruct",
                "qwen",
                false,
            ),
            (
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            if *template != "llama3" {
                assert!(!yaml_line(&case_recipe, "template: llama3"), "{train}");
            }
            let case_export = std::fs::read_to_string(case_out.join("export.yaml")).unwrap();
            assert!(
                yaml_line(&case_export, &template_line),
            ...
            let case_prepare = std::fs::read_to_string(case_out.join("PREPARE.md")).unwrap();
            assert_eq!(
                case_next.contains(LLAMA32_LORA_NOTE),
                *note,
                "{train}\n{case_next}"
            );
            assert_eq!(
                case_prepare.contains(LLAMA32_LORA_NOTE),
                *note,
                "{train}\n{case_prepare}"
            );
            assert!(!case_next.contains(LLAMA32_QLORA_NOTE), "{train}");
            assert!(!case_next.contains("READY_FOR_LIVE_TEST: yes"), "{train}");
            let qlora_case = root.join(format!("qlora-case-{idx}"));
            run(
                LLAMAFACTORY_QLORA_ID,
            ...
            .unwrap();
            let qlora_case_next = std::fs::read_to_string(qlora_case.join("NEXT.md")).unwrap();
            let qlora_case_prepare =
                std::fs::read_to_string(qlora_case.join("PREPARE.md")).unwrap();
            assert!(
                !qlora_case_next.contains(LLAMA32_LORA_NOTE),
                "{train}\n{qlora_case_next}"
            );
            assert!(
                !qlora_case_prepare.contains(LLAMA32_LORA_NOTE),
                "{train}\n{qlora_case_prepare}"
            );
            assert_eq!(
                qlora_case_next.contains(LLAMA32_QLORA_NOTE),
                *note,
                "{train}\n{qlora_case_next}"
            );
        }
        ...
        let official_next = std::fs::read_to_string(official.join("NEXT.md")).unwrap();
        assert!(official_next.contains(LLAMA32_LORA_NOTE), "{official_next}");
        assert!(!official_next.contains(LLAMA32_QLORA_NOTE), "{official_next}");
        assert!(
            !official_next.contains("READY_FOR_LIVE_TEST: yes"),
            "{official_next}"
        );
    ...
    fn gemma2_qlora_prepare_emits_template_bnb_and_keeps_the_seat_split() {
        ...
        for text in [&next, &prepare_md] {
            ...
            assert!(text.contains("quantization_method bnb"), "{text}");
            assert!(text.contains("quantization_bit 4"), "{text}");
            assert!(!text.contains(GEMMA_LORA_NOTE), "{text}");
            assert!(!text.contains("READY_FOR_LIVE_TEST: yes"), "{text}");
        }
        assert!(!out.join("train.py").exists());
        ...
        assert!(lora_next.contains(GEMMA_LORA_NOTE), "{lora_next}");
        assert!(lora_prepare.contains(GEMMA_LORA_NOTE), "{lora_prepare}");
        assert!(
            !lora_next.contains("quantization_bit: 4")
                && !lora_next.contains("quantization_method: bnb"),
            "{lora_next}"
        );
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            let lora_case_prepare = std::fs::read_to_string(lora_case.join("PREPARE.md")).unwrap();
            assert!(
                !lora_case_next.contains(GEMMA_NOTE),
                "{train}\n{lora_case_next}"
            );
            assert_eq!(
                lora_case_next.contains(GEMMA_LORA_NOTE),
                *note,
                "{train}\n{lora_case_next}"
            );
            assert_eq!(
                lora_case_prepare.contains(GEMMA_LORA_NOTE),
                *note,
                "{train}\n{lora_case_prepare}"
            );
        }
    ...
    #[test]
    fn gemma2_lora_prepare_emits_template_and_keeps_the_seat_split() {
        const GEMMA_LORA_NOTE: &str =
            "Reproduce target on the unquantized LoRA card, the non-quant twin of the Gemma-2 Instruct QLoRA prepare.";
        const GEMMA_QLORA_NOTE: &str =
            "Reproduce target beside Phi-3, Llama-3.2, and Qwen LoRA/QLoRA.";
        let root = tmp("gemma2-instruct-lora");
        let pack_path = repo_root().join("examples/fixtures/gemma2-instruct-lora.pack.json");
        let pack: PackManifest =
            serde_json::from_str(&std::fs::read_to_string(&pack_path).unwrap()).unwrap();
        assert_eq!(pack.id, "gemma2-instruct-lora");
        assert_eq!(
            pack.train_base_model.as_deref(),
            Some("google/gemma-2-2b-it")
        );
        ...
        assert!(
            recipe
                .lines()
                .any(|line| line.trim() == "finetuning_type: lora"),
            "{recipe}"
        );
        assert!(
            !recipe.contains("quantization_bit") && !recipe.contains("quantization_method"),
        ...
        for text in [&next, &prepare_md] {
            assert!(text.contains(GEMMA_LORA_NOTE), "{text}");
            assert!(
                text.contains("uses LLaMA-Factory template gemma2."),
                "{text}"
            );
            assert!(text.contains("google/gemma-2-2b-it"), "{text}");
            assert!(
            ...
            assert!(text.contains("lora_rank 8"), "{text}");
            assert!(!text.contains(GEMMA_QLORA_NOTE), "{text}");
            assert!(
                !text.contains("Reproduce target beside Qwen LoRA/QLoRA."),
                "{text}"
            );
        ...
        let qlora_next = std::fs::read_to_string(qlora_out.join("NEXT.md")).unwrap();
        let qlora_prepare = std::fs::read_to_string(qlora_out.join("PREPARE.md")).unwrap();
        assert!(qlora_next.contains(GEMMA_QLORA_NOTE), "{qlora_next}");
        assert!(qlora_prepare.contains(GEMMA_QLORA_NOTE), "{qlora_prepare}");
        assert!(!qlora_next.contains(GEMMA_LORA_NOTE), "{qlora_next}");
        assert!(!qlora_prepare.contains(GEMMA_LORA_NOTE), "{qlora_prepare}");
        ...
        let nested_next = std::fs::read_to_string(nested_out.join("NEXT.md")).unwrap();
        let nested_prepare = std::fs::read_to_string(nested_out.join("PREPARE.md")).unwrap();
        assert!(nested_next.contains(GEMMA_LORA_NOTE), "{nested_next}");
        assert!(nested_prepare.contains(GEMMA_LORA_NOTE), "{nested_prepare}");
        assert!(!nested_next.contains(GEMMA_QLORA_NOTE), "{nested_next}");
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            let case_prepare = std::fs::read_to_string(case_out.join("PREPARE.md")).unwrap();
            assert_eq!(
                case_next.contains(GEMMA_LORA_NOTE),
                *note,
                "{train}\n{case_next}"
            );
            assert_eq!(
                case_prepare.contains(GEMMA_LORA_NOTE),
                *note,
                "{train}\n{case_prepare}"
            );
            assert!(!case_next.contains(GEMMA_QLORA_NOTE), "{train}");
            assert!(!case_next.contains("READY_FOR_LIVE_TEST: yes"), "{train}");
            let qlora_case = root.join(format!("qlora-case-{idx}"));
            run(
                LLAMAFACTORY_QLORA_ID,
            ...
            let qlora_case_next = std::fs::read_to_string(qlora_case.join("NEXT.md")).unwrap();
            assert!(
                !qlora_case_next.contains(GEMMA_LORA_NOTE),
                "{train}\n{qlora_case_next}"
            );
        }
        ...
        let official_next = std::fs::read_to_string(official.join("NEXT.md")).unwrap();
        assert!(official_next.contains(GEMMA_LORA_NOTE), "{official_next}");
        assert!(!official_next.contains(GEMMA_QLORA_NOTE), "{official_next}");
        assert!(
            !official_next.contains("READY_FOR_LIVE_TEST: yes"),
            "{official_next}"
        );
    ...
    #[test]
    fn mistral_lora_prepare_emits_template_and_keeps_the_seat_split() {
        const MISTRAL_LORA_NOTE: &str =
            "Reproduce target on the unquantized LoRA card, the non-quant twin of the Mistral Instruct QLoRA prepare.";
        const MISTRAL_QLORA_NOTE: &str =
            "Reproduce target beside Phi-3, Llama-3.2, Gemma-2, and Qwen LoRA/QLoRA.";
        let root = tmp("mistral-instruct-lora");
        let pack_path = repo_root().join("examples/fixtures/mistral-instruct-lora.pack.json");
        let pack: PackManifest =
            serde_json::from_str(&std::fs::read_to_string(&pack_path).unwrap()).unwrap();
        assert_eq!(pack.id, "mistral-instruct-lora");
        assert_eq!(
            pack.train_base_model.as_deref(),
            Some("mistralai/Mistral-7B-Instruct-v0.3")
        );
        ...
        let recipe = std::fs::read_to_string(out.join("recipe.yaml")).unwrap();
        assert!(
            recipe.lines().any(|line| line.trim() == "template: mistral"),
            "{recipe}"
        );
        assert!(
            !recipe
        ...
        let export = std::fs::read_to_string(out.join("export.yaml")).unwrap();
        assert!(
            export.lines().any(|line| line.trim() == "template: mistral"),
            "{export}"
        );
        assert!(
            !export.contains("quantization_bit") && !export.contains("quantization_method"),
        ...
        let prepare_md = std::fs::read_to_string(out.join("PREPARE.md")).unwrap();
        for text in [&next, &prepare_md] {
            assert!(text.contains(MISTRAL_LORA_NOTE), "{text}");
            assert!(
                text.contains("uses LLaMA-Factory template mistral."),
                "{text}"
            );
            assert!(text.contains("mistralai/Mistral-7B-Instruct-v0.3"), "{text}");
            assert!(text.contains("mistralai/Mistral-7B-Instruct-v0.1"), "{text}");
            assert!(text.contains("There is no mistral_7 template."), "{text}");
            assert!(
                text.contains("omits quantization_bit and quantization_method"),
                "{text}"
            );
            assert!(text.contains("lora_rank 8"), "{text}");
            assert!(text.contains("packing false"), "{text}");
            assert!(!text.contains(MISTRAL_QLORA_NOTE), "{text}");
            assert!(
                !text.contains("Reproduce target beside Qwen LoRA/QLoRA."),
                "{text}"
            );
        ...
        let qlora_next = std::fs::read_to_string(qlora_out.join("NEXT.md")).unwrap();
        let qlora_prepare = std::fs::read_to_string(qlora_out.join("PREPARE.md")).unwrap();
        assert!(qlora_next.contains(MISTRAL_QLORA_NOTE), "{qlora_next}");
        assert!(qlora_prepare.contains(MISTRAL_QLORA_NOTE), "{qlora_prepare}");
        assert!(!qlora_next.contains(MISTRAL_LORA_NOTE), "{qlora_next}");
        assert!(!qlora_prepare.contains(MISTRAL_LORA_NOTE), "{qlora_prepare}");
        ...
        assert!(
            nested_recipe.lines().any(|line| line.trim() == "lora_rank: 8"),
            "{nested_recipe}"
        );
        assert!(
            nested_recipe
        ...
        let nested_next = std::fs::read_to_string(nested_out.join("NEXT.md")).unwrap();
        let nested_prepare = std::fs::read_to_string(nested_out.join("PREPARE.md")).unwrap();
        assert!(nested_next.contains(MISTRAL_LORA_NOTE), "{nested_next}");
        assert!(nested_prepare.contains(MISTRAL_LORA_NOTE), "{nested_prepare}");
        assert!(!nested_next.contains(MISTRAL_QLORA_NOTE), "{nested_next}");

        let cases = [
            ("mistralai/Mistral-7B-Instruct-v0.1", "mistral", true),
            ("mistralai/Mistral-7B-Instruct-v0.2", "mistral", true),
            ("mistralai/Mistral-7B-Instruct-v0.3", "mistral", true),
            ("mistralai/Mistral-7B-v0.1", "mistral", false),
            ("mistralai/Mistral-7B-v0.3", "mistral", false),
            ("alpindale/Mistral-7B-v0.2-hf", "mistral", false),
            ("teknium/OpenHermes-2.5-Mistral-7B", "mistral", false),
            ...
            (
                "/home/user/.cache/huggingface/hub/models--mistralai--Mistral-7B-Instruct-v0.1/snapshots/abc123def789",
                "mistral",
                true,
            ),
            (
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            assert!(
                case_recipe.lines().any(|line| line.trim() == "lora_rank: 8"),
                "{train}"
            );
            assert!(
                case_recipe
            ...
            let case_prepare = std::fs::read_to_string(case_out.join("PREPARE.md")).unwrap();
            assert_eq!(
                case_next.contains(MISTRAL_LORA_NOTE),
                *note,
                "{train}\n{case_next}"
            );
            assert_eq!(
                case_prepare.contains(MISTRAL_LORA_NOTE),
                *note,
                "{train}\n{case_prepare}"
            );
            assert!(!case_next.contains(MISTRAL_QLORA_NOTE), "{train}");
            assert!(!case_next.contains("READY_FOR_LIVE_TEST: yes"), "{train}");
            let qlora_case = root.join(format!("qlora-case-{idx}"));
            run(
                LLAMAFACTORY_QLORA_ID,
            ...
            let qlora_case_prepare =
                std::fs::read_to_string(qlora_case.join("PREPARE.md")).unwrap();
            assert!(
                !qlora_case_next.contains(MISTRAL_LORA_NOTE),
                "{train}\n{qlora_case_next}"
            );
            assert!(
                !qlora_case_prepare.contains(MISTRAL_LORA_NOTE),
                "{train}\n{qlora_case_prepare}"
            );
            assert_eq!(
                qlora_case_next.contains(MISTRAL_QLORA_NOTE),
                *note,
                "{train}\n{qlora_case_next}"
            );
        }
        ...
        let official_next = std::fs::read_to_string(official.join("NEXT.md")).unwrap();
        assert!(official_next.contains(MISTRAL_LORA_NOTE), "{official_next}");
        assert!(!official_next.contains(MISTRAL_QLORA_NOTE), "{official_next}");
        assert!(
            !official_next.contains("READY_FOR_LIVE_TEST: yes"),
            "{official_next}"
        );
    ...
    #[test]
    fn glm4_chat_qlora_prepare_emits_template_bnb_and_keeps_the_seat_split() {
        const GLM4_NOTE: &str =
        ...
        for text in [&next, &prepare_md] {
            ...
            assert!(text.contains("constants.py registers that group"), "{text}");
        ...
        let lora_recipe = std::fs::read_to_string(lora_out.join("recipe.yaml")).unwrap();
        assert!(yaml_line(&lora_recipe, "template: glm4"), "{lora_recipe}");
        assert!(yaml_line(&lora_recipe, "lora_rank: 8"), "{lora_recipe}");
        assert!(yaml_line(&lora_recipe, "packing: false"), "{lora_recipe}");
        assert!(
            !lora_recipe.contains("quantization_bit")
        ...
        let nested_estate = with_train_base(
            seated.clone(),
            "./weights/zai-org/glm-4-9b-chat/weights",
        );
        let nested_out = root.join("nested-9b");
        let nested = run(
            LLAMAFACTORY_QLORA_ID,
        ...
        let nested_train = nested.train_base_model.as_deref().unwrap();
        assert!(
            nested_train.ends_with("/weights/zai-org/glm-4-9b-chat/weights"),
            "{nested_train}"
        );
        assert_eq!(nested.base_model, "llama3");
        let nested_recipe = std::fs::read_to_string(nested_out.join("recipe.yaml")).unwrap();
        ...
        let nested_next = std::fs::read_to_string(nested_out.join("NEXT.md")).unwrap();
        assert!(nested_next.contains(GLM4_NOTE), "{nested_next}");
        assert!(!nested_next.contains(DISTILL_NOTE), "{nested_next}");
        assert!(!nested_next.contains(QWEN25_NOTE), "{nested_next}");
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            let case_next = std::fs::read_to_string(case_out.join("NEXT.md")).unwrap();
            let case_prepare = std::fs::read_to_string(case_out.join("PREPARE.md")).unwrap();
            assert_eq!(case_next.contains(GLM4_NOTE), *note, "{train}\n{case_next}");
            assert_eq!(
                case_prepare.contains(GLM4_NOTE),
                *note,
                "{train}\n{case_prepare}"
            );
            ...
            if train.ends_with("Qwen2.5-0.5B-Instruct") {
                assert!(case_next.contains(QWEN25_NOTE), "{train}\n{case_next}");
                assert!(!case_next.contains(GLM4_NOTE), "{train}\n{case_next}");
            }
            ...
            assert!(yaml_line(&lora_case_recipe, "lora_rank: 8"), "{train}");
            assert!(yaml_line(&lora_case_recipe, "packing: false"), "{train}");
        }
    ...
    fn segment_is_qwen25_instruct_matches_the_text_group() {
        ...
        for segment in instruct {
            assert!(
                segment_is_qwen25_instruct(segment),
                "{segment}"
            );
            assert_eq!(llamafactory_template(segment), "qwen", "{segment}");
        }
        let excluded = [
            "Qwen2.5-0.5B",
            "Qwen/Qwen2.5-0.5B",
            ...
            "Qwen/Qwen2.5-32B",
            "Qwen/Qwen2.5-72B",
            "Qwen/Qwen2.5-7B-Thinking",
            "Qwen/Qwen2.5-7B-Instruct-Thinking",
            "Qwen/Qwen2.5-Coder-0.5B-Instruct",
        ...
        assert_eq!(
            llamafactory_template("Qwen/Qwen2.5-Coder-7B-Instruct"),
            "qwen"
        );
        assert_eq!(llamafactory_template("Qwen/Qwen2.5-0.5B"), "qwen");
        assert_eq!(
            llamafactory_template("Qwen/Qwen3-4B-Instruct-2507"),
            "qwen3_nothink"
        );
        assert_ne!(
        ...
        assert_ne!(llamafactory_template("Qwen/Qwen2.5-0.5B-Instruct"), "qwen3");
    }

    #[test]
    fn qwen25_instruct_qlora_prepare_emits_template_bnb_and_keeps_the_seat_split() {
        const QWEN25_NOTE: &str =
            "Reproduce target beside Phi-3, Llama-3.2, Gemma-2, Mistral, and Qwen3 Instruct QLoRA.";
        const QWEN3_NOTE: &str =
            "Reproduce target beside Phi-3, Llama-3.2, Gemma-2, Mistral, and Qwen2.x LoRA/QLoRA.";
        ...
        let pack: PackManifest =
            serde_json::from_str(&std::fs::read_to_string(&pack_path).unwrap()).unwrap();
        assert_eq!(pack.id, "qwen25-instruct");
        assert_eq!(
            pack.train_base_model.as_deref(),
            Some("Qwen/Qwen2.5-0.5B-Instruct")
        );
        ...
        assert!(
            !recipe.lines().any(|line| line.trim() == "template: qwen3"),
            "{recipe}"
        );
        assert!(recipe.contains("quantization_bit: 4"), "{recipe}");
        assert!(recipe.contains("quantization_method: bnb"), "{recipe}");
        assert!(
            recipe.lines().any(|line| line.trim() == "lora_rank: 16"),
            "{recipe}"
        );
        assert!(
            recipe.lines().any(|line| line.trim() == "packing: true"),
            "{recipe}"
        );
        assert!(
        ...
        for text in [&next, &prepare_md] {
            assert!(text.contains(QWEN25_NOTE), "{text}");
            assert!(
                text.contains("uses LLaMA-Factory template qwen."),
                "{text}"
            );
        ...
        let lora_recipe = std::fs::read_to_string(lora_out.join("recipe.yaml")).unwrap();
        assert!(
            lora_recipe.lines().any(|line| line.trim() == "template: qwen"),
            "{lora_recipe}"
        );
        assert!(
            lora_recipe.lines().any(|line| line.trim() == "lora_rank: 8"),
        ...
        let qwen3_pack_path = repo_root().join("examples/fixtures/qwen3-instruct.pack.json");
        let qwen3_pack: PackManifest =
            serde_json::from_str(&std::fs::read_to_string(&qwen3_pack_path).unwrap()).unwrap();
        let qwen3_out = root.join("qwen3");
        let qwen3 = run(
            LLAMAFACTORY_QLORA_ID,
            &qwen3_pack,
            &estate,
        ...
        .unwrap();
        assert_eq!(
            qwen3.train_base_model.as_deref(),
            Some("Qwen/Qwen3-4B-Instruct-2507")
        );
        assert_eq!(qwen3.base_model, "llama3");
        let qwen3_recipe = std::fs::read_to_string(qwen3_out.join("recipe.yaml")).unwrap();
        assert!(
            qwen3_recipe
                .lines()
                .any(|line| line.trim() == "template: qwen3_nothink"),
        ...
        let qwen3_next = std::fs::read_to_string(qwen3_out.join("NEXT.md")).unwrap();
        let qwen3_prepare = std::fs::read_to_string(qwen3_out.join("PREPARE.md")).unwrap();
        assert!(qwen3_next.contains(QWEN3_NOTE), "{qwen3_next}");
        ...
        assert!(
            nested_recipe
                .lines()
                .any(|line| line.trim() == "template: qwen"),
            "{nested_recipe}"
        );
        assert!(
            nested_recipe.contains("quantization_method: bnb"),
        ...
        let cases = [
            ...
            ("Qwen/Qwen2.5-VL-7B-Instruct", "qwen", false),
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            assert!(case_recipe.contains("quantization_bit: 4"), "{train}");
            assert!(
                case_recipe.lines().any(|line| line.trim() == "lora_rank: 16"),
                "{train}"
            );
            assert!(
                case_recipe.lines().any(|line| line.trim() == "packing: true"),
                "{train}"
            );
            ...
            let case_next = std::fs::read_to_string(case_out.join("NEXT.md")).unwrap();
            let case_prepare = std::fs::read_to_string(case_out.join("PREPARE.md")).unwrap();
            assert_eq!(case_next.contains(QWEN25_NOTE), *note, "{train}\n{case_next}");
            assert_eq!(
                case_prepare.contains(QWEN25_NOTE),
                *note,
                "{train}\n{case_prepare}"
            );
            if *template == "qwen3_nothink" {
            ...
            assert!(
                lora_case_recipe.lines().any(|line| line.trim() == template_line),
                "{train}\n{lora_case_recipe}"
            );
        }
    ...
    fn qwen3_instruct_qlora_prepare_emits_template_bnb_and_keeps_the_seat_split() {
        ...
        const QWEN3_LORA_NOTE: &str =
            "Reproduce target on the unquantized LoRA card, the non-quant twin of the Qwen3 Instruct QLoRA prepare.";
        let root = tmp("qwen3-instruct-qlora");
        let pack_path = repo_root().join("examples/fixtures/qwen3-instruct.pack.json");
        let pack: PackManifest =
            serde_json::from_str(&std::fs::read_to_string(&pack_path).unwrap()).unwrap();
        assert_eq!(
            pack.train_base_model.as_deref(),
        ...
        assert!(recipe.contains("quantization_method: bnb"), "{recipe}");
        assert!(
            recipe.contains("Qwen3 Instruct-2507 (Qwen3-4B-Instruct-2507, Qwen3-30B-A3B-Instruct-2507, Qwen3-235B-A22B-Instruct-2507) and Qwen3-Next-80B-A3B-Instruct are the qwen3_nothink reproduce target."),
            "{recipe}"
        );
        assert!(
        ...
        assert!(!export.contains("quantization_bit"), "{export}");
        assert!(
            export.contains("model_name_or_path: \"Qwen/Qwen3-4B-Instruct-2507\""),
            "{export}"
        );
        let next = std::fs::read_to_string(out.join("NEXT.md")).unwrap();
        let prepare_md = std::fs::read_to_string(out.join("PREPARE.md")).unwrap();
        for text in [&next, &prepare_md] {
            assert!(text.contains(QWEN3_NOTE), "{text}");
            assert!(
                text.contains("uses LLaMA-Factory template qwen3_nothink."),
                "{text}"
            );
            assert!(text.contains("Qwen/Qwen3-4B-Instruct-2507"), "{text}");
            assert!(
                text.contains("examples/train_lora/qwen3_lora_sft.yaml"),
                "{text}"
            );
            assert!(
                !text.contains("Reproduce target beside Qwen LoRA/QLoRA."),
        ...
        let lora_prepare = std::fs::read_to_string(lora_out.join("PREPARE.md")).unwrap();
        assert!(!lora_next.contains(QWEN3_NOTE), "{lora_next}");
        assert!(!lora_prepare.contains(QWEN3_NOTE), "{lora_prepare}");
        assert!(lora_next.contains(QWEN3_LORA_NOTE), "{lora_next}");
        assert!(lora_prepare.contains(QWEN3_LORA_NOTE), "{lora_prepare}");
        assert!(
            !lora_next.contains("quantization_bit: 4")
                && !lora_next.contains("quantization_method: bnb"),
            "{lora_next}"
        ...
        for bad in ["llama3", "llama3:latest", "./llama3", "../llama3"] {
            ...
            assert!(!err.to_string().contains("Qwen/Qwen3"), "{bad}: {err}");
            assert!(!bad_out.exists(), "{bad}");
        }
        for bad in ["./qwen3-4b-instruct-2507", "/opt/hf/qwen3-4b-instruct-2507"] {
            let bad_estate = with_train_base(seated.clone(), bad);
            let bad_out = root.join(format!("leaf-{}", bad.replace('/', "_")));
            let err = run(
                LLAMAFACTORY_QLORA_ID,
        ...
        let nested_estate = with_train_base(
            seated.clone(),
            "./weights/Qwen/Qwen3-30B-A3B-Instruct-2507/weights",
        );
        let nested_out = root.join("nested-30b");
        let nested = run(
            LLAMAFACTORY_QLORA_ID,
            &bare,
            &nested_estate,
        ...
        let nested_train = nested.train_base_model.as_deref().unwrap();
        assert!(
            nested_train.ends_with("/weights/Qwen/Qwen3-30B-A3B-Instruct-2507/weights"),
            "{nested_train}"
        );
        assert_eq!(nested.base_model, "llama3");
        let nested_recipe = std::fs::read_to_string(nested_out.join("recipe.yaml")).unwrap();
        ...
        let nested_prepare = std::fs::read_to_string(nested_out.join("PREPARE.md")).unwrap();
        assert!(nested_next.contains(QWEN3_NOTE), "{nested_next}");
        assert!(nested_prepare.contains(QWEN3_NOTE), "{nested_prepare}");

        let cases = [
            ("Qwen/Qwen3-30B-A3B-Instruct-2507", "qwen3_nothink", true),
            ("Qwen/Qwen3-235B-A22B-Instruct-2507", "qwen3_nothink", true),
            ("Qwen/Qwen3-Next-80B-A3B-Instruct", "qwen3_nothink", true),
            ("unsloth/Qwen3-4B-Instruct-2507", "qwen3_nothink", true),
            (
                "/home/user/.cache/huggingface/hub/models--Qwen--Qwen3-4B-Instruct-2507/snapshots/abc123def456",
                "qwen3_nothink",
                true,
            ),
            (
            ...
            ("Qwen/Qwen3-4B", "qwen3", false),
            ("Qwen/Qwen3-0.6B", "qwen3", false),
            ("Qwen/Qwen3-4B-Base", "qwen3", false),
            ("Qwen/Qwen3-4B-Thinking-2507", "qwen3", false),
            ("Qwen/Qwen3-30B-A3B-Thinking-2507", "qwen3", false),
            ("Qwen/Qwen3-Next-80B-A3B-Thinking", "qwen3", false),
            ("Qwen/Qwen3-4B-Instruct-Thinking", "qwen3", false),
            ("Qwen/Qwen3-4B-nothink", "qwen3_nothink", false),
            (
                "/tmp/Qwen3-4B-Instruct-2507/Qwen3-4B-Thinking-2507",
                "qwen3",
                false,
            ...
            ("Qwen/Qwen2.5-0.5B-Instruct", "qwen", false),
            ("Qwen/Qwen2.5-7B-Instruct", "qwen", false),
            ("Qwen/Qwen2-0.5B-Instruct", "qwen", false),
            ("Qwen/Qwen3-VL-4B-Instruct", "qwen3_nothink", false),
            ("Qwen/Qwen3-VL-4B-Thinking", "qwen3", false),
            ("Qwen/Qwen3-Omni-30B-A3B-Instruct", "qwen3_nothink", false),
            ("Qwen/Qwen3-Omni-30B-A3B-Thinking", "qwen3", false),
            ("microsoft/Phi-3-mini-4k-instruct", "phi", false),
            ("meta-llama/Llama-3.2-3B-Instruct", "llama3", false),
            ("google/gemma-2-2b-it", "gemma2", false),
            ("mistralai/Mistral-7B-Instruct-v0.3", "mistral", false),
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            let case_prepare = std::fs::read_to_string(case_out.join("PREPARE.md")).unwrap();
            assert_eq!(
                case_next.contains(QWEN3_NOTE),
                *note,
                "{train}\n{case_next}"
            );
            assert_eq!(
                case_prepare.contains(QWEN3_NOTE),
                *note,
                "{train}\n{case_prepare}"
            );
            assert!(!case_next.contains("READY_FOR_LIVE_TEST: yes"), "{train}");
            ...
            let lora_case_recipe = std::fs::read_to_string(lora_case.join("recipe.yaml")).unwrap();
            assert!(
                !lora_case_next.contains(QWEN3_NOTE),
                "{train}\n{lora_case_next}"
            );
            assert!(
                !lora_case_recipe.contains("quantization_bit")
            ...
            assert_eq!(
                lora_case_next.contains(QWEN3_LORA_NOTE),
                *note,
                "{train}\n{lora_case_next}"
            );
            assert_eq!(
                lora_case_prepare.contains(QWEN3_LORA_NOTE),
                *note,
                "{train}\n{lora_case_prepare}"
            );
        }
        ...
        let sacred_base = with_train_base(seated.clone(), "cyera/Qwen3-4B-Instruct-2507");
        let sacred_base_out = root.join("sacred-base");
        let sacred_train = run(
            LLAMAFACTORY_QLORA_ID,
            &bare,
        ...
        let sku_base = with_train_base(
            seated.clone(),
            "/tmp/cell-one-hf/5090/Qwen3-4B-Instruct-2507",
        );
        let sku_out = root.join("sku-base");
        let sku = run(
            LLAMAFACTORY_QLORA_ID,
    ...
    #[test]
    fn qwen3_instruct_lora_prepare_emits_template_and_keeps_the_seat_split() {
        const QWEN3_LORA_NOTE: &str =
            "Reproduce target on the unquantized LoRA card, the non-quant twin of the Qwen3 Instruct QLoRA prepare.";
        const QWEN3_QLORA_NOTE: &str =
            "Reproduce target beside Phi-3, Llama-3.2, Gemma-2, Mistral, and Qwen2.x LoRA/QLoRA.";
        let root = tmp("qwen3-instruct-lora");
        let pack_path = repo_root().join("examples/fixtures/qwen3-instruct-lora.pack.json");
        let pack: PackManifest =
            serde_json::from_str(&std::fs::read_to_string(&pack_path).unwrap()).unwrap();
        assert_eq!(pack.id, "qwen3-instruct-lora");
        assert_eq!(
            pack.train_base_model.as_deref(),
            Some("Qwen/Qwen3-4B-Instruct-2507")
        );
        ...
        assert!(
            recipe.contains("examples/train_lora/qwen3_lora_sft.yaml"),
            "{recipe}"
        );
        assert!(
            recipe.contains("model_name_or_path: \"Qwen/Qwen3-4B-Instruct-2507\""),
        ...
        let qlora_prepare = std::fs::read_to_string(qlora_out.join("PREPARE.md")).unwrap();
        assert!(qlora_next.contains(QWEN3_QLORA_NOTE), "{qlora_next}");
        assert!(qlora_prepare.contains(QWEN3_QLORA_NOTE), "{qlora_prepare}");
        assert!(!qlora_next.contains(QWEN3_LORA_NOTE), "{qlora_next}");
        assert!(!qlora_prepare.contains(QWEN3_LORA_NOTE), "{qlora_prepare}");
        ...
        let nested_prepare = std::fs::read_to_string(nested_out.join("PREPARE.md")).unwrap();
        assert!(nested_next.contains(QWEN3_LORA_NOTE), "{nested_next}");
        assert!(nested_prepare.contains(QWEN3_LORA_NOTE), "{nested_prepare}");
        assert!(!nested_next.contains(QWEN3_QLORA_NOTE), "{nested_next}");
        ...
        for (idx, (train, template, note)) in cases.iter().enumerate() {
            ...
            assert_eq!(
                case_prepare.contains(QWEN3_LORA_NOTE),
                *note,
                "{train}\n{case_prepare}"
            );
            assert!(!case_next.contains(QWEN3_QLORA_NOTE), "{train}");
            assert!(!case_next.contains("READY_FOR_LIVE_TEST: yes"), "{train}");
            let qlora_case = root.join(format!("qlora-case-{idx}"));
            run(
                LLAMAFACTORY_QLORA_ID,
        ...
        let official_next = std::fs::read_to_string(official.join("NEXT.md")).unwrap();
        assert!(official_next.contains(QWEN3_LORA_NOTE), "{official_next}");
        assert!(!official_next.contains(QWEN3_QLORA_NOTE), "{official_next}");
        assert!(
            !official_next.contains("READY_FOR_LIVE_TEST: yes"),
            "{official_next}"
        );
    ...
    fn llamafactory_lora_prepares_unquantized_and_keeps_the_seat_split() {
        ...
        assert!(
            qlora_prepare.contains(
                "Reproduce target beside Phi-3, Llama-3.2, Gemma-2, Mistral, and Qwen2.x LoRA/QLoRA."
            ),
            "{qlora_prepare}"
        );
        assert!(
            !qlora_next.contains("READY_FOR_LIVE_TEST: yes"),
            "{qlora_next}"
        );
        assert!(
            !qlora_next.contains(
        ...
        let instruct_next = std::fs::read_to_string(instruct_out.join("NEXT.md")).unwrap();
        let instruct_prepare = std::fs::read_to_string(instruct_out.join("PREPARE.md")).unwrap();
        assert!(
            !instruct_next.contains(
                "Reproduce target beside Phi-3, Llama-3.2, Gemma-2, Mistral, and Qwen2.x LoRA/QLoRA."
            ),
            "{instruct_next}"
        );
        ...
        let hybrid_next = std::fs::read_to_string(hybrid_out.join("NEXT.md")).unwrap();
        assert!(
            !hybrid_next.contains(
                "Reproduce target on the unquantized LoRA card, the non-quant twin of the Qwen3 Instruct QLoRA prepare."
            ),
            "{hybrid_next}"
        );
        ...
        let snapshot_next = std::fs::read_to_string(snapshot_out.join("NEXT.md")).unwrap();
        assert!(
            snapshot_next.contains(
                "Reproduce target on the unquantized LoRA card, the non-quant twin of the Qwen3 Instruct QLoRA prepare."
            ),
            "{snapshot_next}"
        );
        assert!(
            !snapshot_recipe.contains("quantization_bit")
                && !snapshot_recipe.contains("quantization_method"),
            "{snapshot_recipe}"
        );
