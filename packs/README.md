...
- `estate packs propose` writes `packs/proposed/{id}.proposal.json` after import. `auto_apply` is always false. Jason reviews the diff and edits the estate by hand. `packs accept` copies `source_drivers` into `{id}.enrich-edit.json` and does not rewrite the estate. A tag that does not match `path_counts` refuses before that file is rewritten.
