...
One-box factory skeleton. Proves **A1–A4** on a synthetic Horizon / Research / Sanctum estate. Pause-safe. No live Grok, no Dual PE, no multi-box control, no AI-gateway product.
...
From the repo root (Rust 1.88+, pinned in `rust-toolchain.toml`):
...
- Frontier (`xai_grok`) and local (`local_slm`) are schema-equal bindings. Day 0–30 on `main` may still show placeholders; Day 31–60 on the PR wires them.
...
5. **Vendor hints live only in estate `params`**. Floor-supervisor sources are tested to reject vendor strings.
6. **Canonical JSON SHA-256** of the desired estate is the plan/apply hash.
7. **Apply/drift shipped thin** so pause/rebind has something to converge.
8. **Tool ids are slugs** (`notes-append`), not dotted names.
9. **tiny_http proxy stub** instead of axum/gRPC.
