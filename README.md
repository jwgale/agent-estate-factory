...
Suite: a local-runtime seat (Ollama today, another process tomorrow) and facilitation of train/enrich for purpose-built small models. Ollama is the local-run seat. Integrate the driver that already does the job. `estate enrich prepare` writes artifacts (one driver, or `--all-drivers`). `estate enrich from-pack` does that for an accepted pack. Modelfile `FROM` is the seated model (`params.model` or a model-tag hint), never the binding id `local_slm`. `estate enrich list` reads `.cell/enrich`. `estate enrich import-prepared` writes a `local_slm` binding proposal. `llamafactory-qlora` writes a LLaMA-Factory QLoRA recipe (`estate enrich prepare --job train`) whose `model_name_or_path` is a train base, separate from the Ollama seat tag. `llamafactory-lora` writes the full-precision LoRA recipe and the same `export.yaml` merge card. `--max-steps` and the other gauge knobs write into that recipe. Prepare does not run `llamafactory-cli export`. `axolotl-lora` writes the YAML recipe with that same train base in `base_model`. `import-trained` records the adapter on that same proposal. The factory does not run LLaMA-Factory or Axolotl. `estate enrich apply-proposal` stages that binding for `estate plan` and `estate apply --require-plan`. The source estate is written when that apply succeeds. Packs stay curator edit instructions. Control does not complete. Prepare: [`docs/TRAIN-ENRICH.md`](docs/TRAIN-ENRICH.md). Walks: [`docs/operator-enrich-journeys.md`](docs/operator-enrich-journeys.md).
