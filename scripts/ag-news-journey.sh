#!/usr/bin/env bash
# Opt-in ag_news classify journey. Default is print, train-size 3000, and the local train driver.
# Set AG_NEWS_RUN=1 to execute train, merge, GGUF, Ollama, and eval.
...
BEFORE="$(cksum "$ESTATE")"
echo "== ag-news-journey (local output; not a live PASS) =="
ARGS=(
  classify journey
  --dataset ag_news
