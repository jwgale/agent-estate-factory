#!/usr/bin/env bash
# Print-only GLM-4 Chat QLoRA uniqueness chain.
# Runs the prepare-assert phase, then the seat-print phase, of
# make glm4-chat-journey. If the prepare phase fails, this
# script exits nonzero before the seat print.
# Does not train, merge, convert, seat, or promote.
# Does not run make glm4-chat-lora-journey or make uniqueness-glm-lora.
...
echo "== uniqueness-glm (print-only GLM-4 Chat QLoRA chain) =="
echo "Print-only. READY_FOR_LIVE_TEST: no"
echo "Does not train, merge, convert, seat, or promote."
echo "Chain: prepare-assert, then seat-print (make glm4-chat-journey)."
echo "Does not run make glm4-chat-lora-journey, make uniqueness-glm-lora, make qlora-journey, make uniqueness-full, or make lf-beachhead-prepare."
...
if ! GLM_CARD=llamafactory-qlora GLM4_CHAT_PHASE=prepare make -C "$ROOT" glm4-chat-journey; then
  echo "FAIL  uniqueness-glm (prepare-assert)" >&2
  exit 1
fi

echo
echo "-- seat-print (GLM4_CHAT_PHASE=seat) --"
if ! GLM_CARD=llamafactory-qlora GLM4_CHAT_PHASE=seat make -C "$ROOT" glm4-chat-journey; then
  echo "FAIL  uniqueness-glm (seat-print)" >&2
  exit 1
fi

echo
echo "Live train, live convert, and live seat still need a human host and stay skipped."
echo "PASS  uniqueness-glm (prepare-assert then seat-print; SKIP live train; SKIP live convert; SKIP live seat)"
