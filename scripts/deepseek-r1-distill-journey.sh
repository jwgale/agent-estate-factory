#!/usr/bin/env bash
# DeepSeek-R1-Distill chat family ladder on the LLaMA-Factory cards already
...
# deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B. Template is deepseekr1.
...
DEEPSEEK_CARD="${DEEPSEEK_CARD:-llamafactory-qlora}"
case "$DEEPSEEK_CARD" in
  llamafactory-qlora|llamafactory-lora) ;;
  *)
    echo "FAIL  DEEPSEEK_CARD must be llamafactory-qlora or llamafactory-lora: $DEEPSEEK_CARD" >&2
    exit 1
    ;;
esac

PHASE="${DEEPSEEK_R1_DISTILL_PHASE:-all}"
case "$PHASE" in
  prepare|seat|all) ;;
  *)
    echo "FAIL  DEEPSEEK_R1_DISTILL_PHASE must be prepare, seat, or all (got: $PHASE)" >&2
    exit 1
    ;;
esac

TRAIN_BASE="deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"
SEAT_TAG="llama3"
TEMPLATE="deepseekr1"
QLORA_NOTE="Reproduce target beside Phi-3, Llama-3.2, Gemma-2, Mistral, Qwen2.5 Instruct, and Qwen3 Instruct QLoRA."
LORA_NOTE="Reproduce target on the unquantized LoRA card, the non-quant twin of the DeepSeek-R1-Distill chat QLoRA prepare."

if [[ "$DEEPSEEK_CARD" == "llamafactory-lora" ]]; then
  PACK_REL="examples/fixtures/deepseek-r1-distill-lora.pack.json"
  PACK_ID="deepseek-r1-distill-lora"
  DEFAULT_WORKDIR="${TMPDIR:-/tmp}/cell-one-deepseek-r1-distill-lora-journey"
  BANNER="DeepSeek-R1-Distill chat / LLaMA-Factory LoRA"
else
  PACK_REL="examples/fixtures/deepseek-r1-distill.pack.json"
  PACK_ID="deepseek-r1-distill"
  DEFAULT_WORKDIR="${TMPDIR:-/tmp}/cell-one-deepseek-r1-distill-journey"
  BANNER="DeepSeek-R1-Distill chat / LLaMA-Factory QLoRA"
fi
...
if [[ ! -f "$PACK" ]]; then
  echo "SKIP deepseek-r1-distill-journey (fixture pack missing; not a PASS)"
  exit 0
fi

echo "== deepseek-r1-distill-journey (${BANNER}; print-only; phase=${PHASE}) =="
echo "workdir: $WORKDIR"
echo "card: $DEEPSEEK_CARD"
...
echo "1. estate enrich prepare --driver ${DEEPSEEK_CARD}"
...
echo "   An Ollama tag deepseek-r1:1.5b is a seat tag, not this train base."
...
echo "-- ${DEEPSEEK_CARD} prepare writes the DeepSeek-R1-Distill card --"
PREPARED="$WORKDIR/$DEEPSEEK_CARD"
estate enrich prepare \
  --estate "$SEATED" \
  --pack "$PACK" \
  --driver "$DEEPSEEK_CARD" \
...
if grep -q "^template: qwen$" "$PREPARED/recipe.yaml" "$PREPARED/export.yaml"; then
  echo "FAIL  DeepSeek-R1-Distill chat must not take template qwen" >&2
  exit 1
fi
if [[ "$DEEPSEEK_CARD" == "llamafactory-lora" ]]; then
  ...
  if grep -F -q "$QLORA_NOTE" "$PREPARED/NEXT.md" "$PREPARED/PREPARE.md"; then
    echo "FAIL  DeepSeek LoRA prepare took the QLoRA reproduce note" >&2
    exit 1
  fi
else
  ...
  if grep -F -q "$LORA_NOTE" "$PREPARED/NEXT.md" "$PREPARED/PREPARE.md"; then
    echo "FAIL  DeepSeek QLoRA prepare took the LoRA reproduce note" >&2
    exit 1
  fi
fi
...
python3 - "$PREPARED/prepare.json" "$TRAIN_BASE" "$SEAT_TAG" "$DEEPSEEK_CARD" "$PACK_ID" <<'PY'
import json, sys
...
finish_locked() {
  ...
  if [[ "$BEFORE" != "$after" ]]; then
    echo "FAIL  deepseek-r1-distill-journey rewrote examples/estate.yaml" >&2
    exit 1
  fi
  if [[ -f "$SEATED" ]]; then
  ...
  if [[ "$PACK_BEFORE" != "$pack_after" ]]; then
    echo "FAIL  deepseek-r1-distill-journey rewrote the fixture pack" >&2
    exit 1
  fi
}
...
if [[ "$PHASE" == "prepare" ]]; then
  finish_locked
  echo
  echo "PASS  deepseek-r1-distill-journey (prepare-assert; template ${TEMPLATE}; fixture ${PACK_ID}; SKIP live train; SKIP live convert; SKIP live seat)"
  echo "READY_FOR_LIVE_TEST: no"
  exit 0
fi
...
echo
echo "PASS  deepseek-r1-distill-journey (${BANNER} ladder printed; template ${TEMPLATE}; fixture stubs; SKIP live train; SKIP live convert; SKIP live seat)"
echo "READY_FOR_LIVE_TEST: no"
