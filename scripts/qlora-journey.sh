#!/usr/bin/env bash
# Target C: Qwen / LLaMA-Factory QLoRA operator ladder.
# Prints the five existing commands and checks llamafactory-qlora prepare artifacts.
...
WORKDIR="${WORKDIR:-${TMPDIR:-/tmp}/cell-one-qlora-journey}"
...
if [[ ! -f "$PACK" ]]; then
  echo "SKIP qlora-journey (example pack missing; not a PASS)"
  exit 0
fi

echo "== qlora-journey (Target C: Qwen / LLaMA-Factory QLoRA; print-only) =="
...
echo "   estate enrich merge-adapt prints llamafactory-cli export <prepared>/export.yaml"
...
if [[ "$BEFORE" != "$AFTER" ]]; then
  echo "FAIL  qlora-journey rewrote examples/estate.yaml"
  exit 1
fi

echo
echo "Filled NEXT.md lines (not executed):"
grep -E '^(llamafactory-cli (train|export) |python3 convert_hf_to_gguf.py |estate enrich (gguf-convert|local-seat|import-trained) )' "$PREPARED/NEXT.md"
echo
echo "PASS  qlora-journey (Target C ladder printed; prepare artifacts checked; SKIP live train)"
echo "READY_FOR_LIVE_TEST: no"
