#!/usr/bin/env bash
# Unsloth QLoRA optional NEXT path. Print-only.
# Prepares unsloth-qlora on a throwaway copy of examples/estate.yaml
...
PHASE="${UNSLOTH_QLORA_PHASE:-all}"
case "$PHASE" in
  prepare|seat|all) ;;
  *)
    echo "FAIL  UNSLOTH_QLORA_PHASE must be prepare, seat, or all (got: $PHASE)" >&2
    exit 1
    ;;
esac
...
WORKDIR="${WORKDIR:-${TMPDIR:-/tmp}/cell-one-unsloth-qlora-journey}"
...
finish() {
  ...
  if [[ "$BEFORE" != "$after" ]]; then
    echo "FAIL  unsloth-qlora-journey rewrote examples/estate.yaml" >&2
    exit 1
  fi
...
if [[ ! -f "$PACK" ]]; then
  echo "SKIP unsloth-qlora-journey (example pack missing; not a PASS)"
  exit 0
fi

echo "== unsloth-qlora-journey (optional Unsloth QLoRA NEXT path; print-only; phase=${PHASE}) =="
...
echo "1. estate enrich prepare --driver unsloth-qlora"
...
echo "-- unsloth-qlora prepare writes the operator-owned handoff --"
PREPARED="$WORKDIR/unsloth-qlora"
...
if grep -q "train_unsloth.py" "$PREPARED/UNSLOTH.md" "$PREPARED/PREPARE.md" "$PREPARED/NEXT.md" "$PREPARED/prepare.json"; then
  echo "FAIL  unsloth-qlora must not name a training script" >&2
  exit 1
fi
...
if [[ "$mlx_shape_rc" -eq 0 ]]; then
  echo "FAIL  merge-adapt must refuse adapters.safetensors on unsloth-qlora" >&2
  exit 1
fi
...
if [[ "$PHASE" == "prepare" ]]; then
  finish
  echo
  echo "PASS  unsloth-qlora-journey (prepare-assert; UNSLOTH.md handoff checked; refuse:adapter; refuse:seat; SKIP live train; SKIP live convert; SKIP live seat)"
  echo "READY_FOR_LIVE_TEST: no"
  exit 0
fi
...
if [[ "$adapter_seat_rc" -eq 0 ]]; then
  echo "FAIL  local-seat --adapter must refuse on unsloth-qlora" >&2
  exit 1
fi
...
echo
echo "PASS  unsloth-qlora-journey (Unsloth QLoRA handoff checked; merge, convert, seat, and import printed; refuse:adapter; refuse:seat; refuse:tokenizer; SKIP live train; SKIP live convert; SKIP live seat)"
echo "READY_FOR_LIVE_TEST: no"
