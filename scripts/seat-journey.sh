...
echo "   Write that file from the printed contents before ollama create."
...
grep -q "local-seat is print-only" "$WORKDIR/logs/seat-gguf.out"
grep -q "does not write ${PREPARED}/Modelfile" "$WORKDIR/logs/seat-gguf.out"
grep -q "Write that file from the printed contents before ollama create" "$WORKDIR/logs/seat-gguf.out"
