#!/usr/bin/env bash
# Axolotl LoRA popular-config parity. Print-only.
# Prepares axolotl-lora on a throwaway copy of examples/estate.yaml
# (seat tag llama3, train base Qwen/Qwen2.5-0.5B-Instruct, overnight pack).
# Asserts axolotl.yml matches the documented examples/llama-3/lora-1b.yml card.
# Phase prepare stops after refuse:train-base, refuse:adapter, and refuse:seat
# on missing paths. Phase seat (and the default phase all) then prints
# merge-adapt, gguf-convert, local-seat, and import-trained against fixture
# stubs, including refuse:tokenizer on a Qwen-shaped merged directory.
# Does not install Axolotl, does not run axolotl, does not merge, does not
...
PHASE="${AXOLOTL_LORA_PHASE:-all}"
case "$PHASE" in
  prepare|seat|all) ;;
  *)
    echo "FAIL  AXOLOTL_LORA_PHASE must be prepare, seat, or all (got: $PHASE)" >&2
    exit 1
    ;;
esac
...
WORKDIR="${WORKDIR:-${TMPDIR:-/tmp}/cell-one-axolotl-lora-journey}"
...
finish() {
  ...
  if [[ "$BEFORE" != "$after" ]]; then
    echo "FAIL  axolotl-lora-journey rewrote examples/estate.yaml" >&2
    exit 1
  fi
...
if [[ ! -f "$PACK" ]]; then
  echo "SKIP axolotl-lora-journey (example pack missing; not a PASS)"
  exit 0
fi

echo "== axolotl-lora-journey (Axolotl LoRA popular-config parity; print-only; phase=${PHASE}) =="
...
echo "1. estate enrich prepare --driver axolotl-lora"
echo "   Seat tag stays the Ollama id. Train base is a Hugging Face repo id."
echo "   Example seat ${SEAT_TAG}. Train base ${TRAIN_BASE}."
echo "   axolotl.yml matches examples/llama-3/lora-1b.yml:"
echo "   adapter lora, load_in_8bit false, load_in_4bit false, sequence_len 2048, lora_r 16."
echo "   base_model is that train base."
echo "2. estate enrich merge-adapt --prepared <prepared> --adapter <prepared>/outputs"
echo "   Prints axolotl merge-lora, without --dequant."
echo "   Axolotl writes <prepared>/outputs/merged. This factory does not merge."
echo "   The adapter stub is adapter_config.json."
echo "3. estate enrich gguf-convert --prepared <prepared> --weights <prepared>/outputs/merged"
...
echo "   Prints python3 convert_hf_to_gguf.py ... --outfile <prepared>/outputs/merged.gguf --outtype auto"
echo "4. estate enrich local-seat --prepared <prepared> --weights <prepared>/outputs/merged.gguf"
...
echo "5. estate enrich import-trained --adapter <prepared>/outputs/merged.gguf"
...
echo "-- axolotl-lora prepare writes the bf16 LoRA card --"
PREPARED="$WORKDIR/axolotl-lora"
estate enrich prepare \
  --estate "$SEATED" \
  --pack "$PACK" \
  --driver axolotl-lora \
...
test -f "$PREPARED/axolotl.yml"
test -f "$PREPARED/dataset.jsonl"
test -f "$PREPARED/PREPARE.md"
test -f "$PREPARED/NEXT.md"
test -f "$PREPARED/prepare.json"
if [[ -e "$PREPARED/recipe.yaml" || -e "$PREPARED/export.yaml" || -e "$PREPARED/train_unsloth.py" || -e "$PREPARED/convert_hf_to_gguf.py" || -e "$PREPARED/outputs" || -e "$PREPARED/export" || -e "$PREPARED/export.gguf" ]]; then
  echo "FAIL  prepare must not invent a LLaMA-Factory card, a trainer, a converter, or weights" >&2
  exit 1
fi

require "base_model: \"${TRAIN_BASE}\"" "$PREPARED/axolotl.yml"
require "load_in_8bit: false" "$PREPARED/axolotl.yml"
require "load_in_4bit: false" "$PREPARED/axolotl.yml"
require "adapter: lora" "$PREPARED/axolotl.yml"
require "sequence_len: 2048" "$PREPARED/axolotl.yml"
require "micro_batch_size: 2" "$PREPARED/axolotl.yml"
require "gradient_accumulation_steps: 2" "$PREPARED/axolotl.yml"
require "lora_r: 16" "$PREPARED/axolotl.yml"
require "lora_alpha: 32" "$PREPARED/axolotl.yml"
require "lora_dropout: 0.05" "$PREPARED/axolotl.yml"
require "lora_target_linear: true" "$PREPARED/axolotl.yml"
require "num_epochs: 1" "$PREPARED/axolotl.yml"
require "optimizer: adamw_8bit" "$PREPARED/axolotl.yml"
require "learning_rate: 0.0002" "$PREPARED/axolotl.yml"
require "warmup_ratio: 0.1" "$PREPARED/axolotl.yml"
require "sample_packing: true" "$PREPARED/axolotl.yml"
require "val_set_size: 0.0" "$PREPARED/axolotl.yml"
require "evals_per_epoch: 0" "$PREPARED/axolotl.yml"
require "saves_per_epoch: 1" "$PREPARED/axolotl.yml"
require "output_dir: \"${PREPARED}/outputs\"" "$PREPARED/axolotl.yml"
require "type: alpaca" "$PREPARED/axolotl.yml"
require "ds_type: json" "$PREPARED/axolotl.yml"
require "examples/llama-3/lora-1b.yml" "$PREPARED/axolotl.yml"
if grep -q '^adapter: qlora$' "$PREPARED/axolotl.yml"; then
  echo "FAIL  axolotl-lora must not write adapter qlora" >&2
  exit 1
fi
if grep -q '^load_in_4bit: true$' "$PREPARED/axolotl.yml"; then
  echo "FAIL  axolotl-lora must not write load_in_4bit true" >&2
  exit 1
fi
if grep -q -- '--dequant' "$PREPARED/axolotl.yml" "$PREPARED/PREPARE.md" "$PREPARED/NEXT.md"; then
  echo "FAIL  axolotl-lora must not print --dequant" >&2
  exit 1
fi
if grep -E '^(attn_implementation|flash_attention|max_steps):' "$PREPARED/axolotl.yml"; then
  echo "FAIL  axolotl-lora must leave flash attention and max_steps unset" >&2
  exit 1
fi
if grep -q "quantization_bit" "$PREPARED/axolotl.yml"; then
  echo "FAIL  axolotl.yml must not use the LLaMA-Factory quantization key" >&2
  exit 1
fi
if grep -q "axolotl train" "$PREPARED/prepare.json"; then
...
require "axolotl train axolotl.yml" "$PREPARED/PREPARE.md"
require "axolotl train ${PREPARED}/axolotl.yml" "$PREPARED/NEXT.md"
require "examples/llama-3/lora-1b.yml" "$PREPARED/NEXT.md"
require "dataset_mode: scaffold" "$PREPARED/NEXT.md"
require "not training data" "$PREPARED/NEXT.md"
require "axolotl merge-lora ${PREPARED}/axolotl.yml --lora-model-dir=${PREPARED}/outputs" "$PREPARED/NEXT.md"
require "estate enrich merge-adapt --prepared ${PREPARED} --adapter ${PREPARED}/outputs" "$PREPARED/NEXT.md"
require "estate enrich gguf-convert --prepared ${PREPARED} --weights ${PREPARED}/outputs/merged" "$PREPARED/NEXT.md"
require "python3 convert_hf_to_gguf.py ${PREPARED}/outputs/merged --outfile ${PREPARED}/outputs/merged.gguf --outtype auto" "$PREPARED/NEXT.md"
require "estate enrich local-seat --prepared ${PREPARED} --weights ${PREPARED}/outputs/merged" "$PREPARED/NEXT.md"
require "estate enrich import-trained --estate <estate.yaml> --prepared ${PREPARED} --tag ${TAG} --adapter ${PREPARED}/outputs/merged.gguf" "$PREPARED/NEXT.md"
require "Axolotl does not write GGUF" "$PREPARED/NEXT.md"
require "does not install flash attention" "$PREPARED/NEXT.md"
require "standing next step" "$PREPARED/NEXT.md"
require "auto_apply=false" "$PREPARED/NEXT.md"
require "did not run ollama create" "$PREPARED/NEXT.md"
require "does not apply the estate" "$PREPARED/NEXT.md"
require "READY_FOR_LIVE_TEST: no" "$PREPARED/NEXT.md"
require "local-seat is print-only" "$PREPARED/NEXT.md"

python3 - "$PREPARED/prepare.json" "$PREPARED/dataset.jsonl" "$TRAIN_BASE" "$SEAT_TAG" <<'PY'
import json, sys
prepare_path, dataset_path, train_base, seat = sys.argv[1:]
...
for row in rows:
    if "instruction" not in row or "input" not in row or "output" not in row:
        raise SystemExit(f"FAIL  dataset row is not Alpaca: {row}")
    if "messages" in row:
        raise SystemExit("FAIL  dataset row must stay Alpaca and must not use messages")
PY
...
if [[ -e "$PREPARED/outputs" || -e "$PREPARED/outputs/merged" ]]; then
...
estate enrich gguf-convert \
  --prepared "$PREPARED" \
  --weights "$PREPARED/outputs/merged" \
...
if [[ -e "$PREPARED/outputs/merged.gguf" || -e "$PREPARED/outputs/merged" ]]; then
...
estate enrich local-seat \
  --prepared "$PREPARED" \
  --weights "$PREPARED/outputs/merged.gguf" \
...
if [[ -e "$PREPARED/outputs/merged.gguf" ]]; then
...
if [[ "$PHASE" == "prepare" ]]; then
  finish
  echo
  echo "PASS  axolotl-lora-journey (prepare-assert; axolotl.yml card checked; refuse:adapter; refuse:seat; SKIP live train; SKIP live convert; SKIP live seat)"
  echo "READY_FOR_LIVE_TEST: no"
  exit 0
fi
...
if [[ -e "$PREPARED/outputs/adapter_model.safetensors" || -e "$PREPARED/outputs/merged" ]]; then
  echo "FAIL  adapter stub must stay a marker file" >&2
  exit 1
fi

echo "-- merge-adapt prints axolotl merge-lora --"
set +e
estate enrich merge-adapt \
  --prepared "$PREPARED" \
  --adapter "$PREPARED/outputs" \
...
require "axolotl merge-lora ${PREPARED}/axolotl.yml --lora-model-dir=${PREPARED}/outputs" "$WORKDIR/logs/merge.out"
require "This card is bf16 LoRA" "$WORKDIR/logs/merge.out"
require "getting-started merge" "$WORKDIR/logs/merge.out"
if grep -q -- '--dequant' "$WORKDIR/logs/merge.out" "$WORKDIR/logs/merge.err"; then
  echo "FAIL  axolotl-lora merge-adapt printed --dequant" >&2
  exit 1
fi
require "python3 convert_hf_to_gguf.py ${PREPARED}/outputs/merged --outfile ${PREPARED}/outputs/merged.gguf --outtype auto" "$WORKDIR/logs/merge.out"
require "estate enrich gguf-convert --prepared ${PREPARED} --weights ${PREPARED}/outputs/merged" "$WORKDIR/logs/merge.out"
require "estate enrich local-seat --prepared ${PREPARED} --weights ${PREPARED}/outputs/merged" "$WORKDIR/logs/merge.out"
require "merge-adapt did not merge" "$WORKDIR/logs/merge.out"
require "READY_FOR_LIVE_TEST: no" "$WORKDIR/logs/merge.out"
if [[ -e "$PREPARED/outputs/merged" || -e "$PREPARED/outputs/merged.gguf" ]]; then
  echo "FAIL  merge-adapt wrote a merged directory or a GGUF" >&2
  exit 1
fi

echo "-- Qwen-shaped merged tokenizer is refuse:tokenizer --"
mkdir -p "$PREPARED/outputs/merged"
printf '%s\n' '{"model_type":"qwen2","architectures":["Qwen2ForCausalLM"]}' > "$PREPARED/outputs/merged/config.json"
printf '%s\n' 'not-a-real-tensor' > "$PREPARED/outputs/merged/model.safetensors"
printf '%s\n' '{"extra_special_tokens":["<|im_start|>","<|im_end|>"],"tokenizer_class":"Qwen2Tokenizer"}' > "$PREPARED/outputs/merged/tokenizer_config.json"
if [[ -e "$PREPARED/outputs/merged/vocab.json" || -e "$PREPARED/outputs/merged/merges.txt" || -e "$PREPARED/outputs/merged.gguf" || -e "$PREPARED/outputs/merged/tokenizer_config.json.bak" ]]; then
...
if [[ -e "$PREPARED/outputs/merged.gguf" || -e "$PREPARED/outputs/merged/model.gguf" || -e "$PREPARED/outputs/merged/tokenizer_config.json.bak" || -e "$PREPARED/outputs/merged/vocab.json" || -e "$PREPARED/outputs/merged/merges.txt" ]]; then
  echo "FAIL  refuse:tokenizer wrote a GGUF or copied tokenizer files" >&2
  exit 1
fi
if ! grep -q '"extra_special_tokens"' "$PREPARED/outputs/merged/tokenizer_config.json"; then
  echo "FAIL  refuse:tokenizer rewrote tokenizer_config.json" >&2
  exit 1
fi

echo "-- replace the broken tokenizer with the good merged stub --"
rm -rf "$PREPARED/outputs/merged"
mkdir -p "$PREPARED/outputs/merged"
printf '%s\n' '{}' > "$PREPARED/outputs/merged/config.json"
printf '%s\n' 'not-a-real-tensor' > "$PREPARED/outputs/merged/model.safetensors"
if [[ -e "$PREPARED/outputs/merged/tokenizer_config.json" || -e "$PREPARED/outputs/merged/vocab.json" || -e "$PREPARED/outputs/merged/merges.txt" ]]; then
  echo "FAIL  good merged stub must not keep the broken tokenizer" >&2
  exit 1
fi
if ! grep -qx '{}' "$PREPARED/outputs/merged/config.json"; then
  echo "FAIL  good merged stub config.json must stay {}" >&2
  exit 1
fi
if [[ -e "$PREPARED/outputs/merged/adapter_config.json" || -e "$PREPARED/outputs/merged/adapter_model.safetensors" || -e "$PREPARED/outputs/merged/model.gguf" ]]; then
  echo "FAIL  merged stub must be config.json plus a non-adapter safetensors file" >&2
  exit 1
fi
if [[ ! -f "$PREPARED/outputs/adapter_config.json" ]]; then
...
require "python3 convert_hf_to_gguf.py ${PREPARED}/outputs/merged --outfile ${PREPARED}/outputs/merged.gguf --outtype auto" "$WORKDIR/logs/gguf-print.out"
require "estate enrich local-seat --prepared ${PREPARED} --weights ${PREPARED}/outputs/merged.gguf" "$WORKDIR/logs/gguf-print.out"
...
require "estate enrich import-trained --estate <estate.yaml> --prepared ${PREPARED} --tag ${TAG} --adapter ${PREPARED}/outputs/merged.gguf" "$WORKDIR/logs/gguf-print.out"
require "Axolotl does not write GGUF" "$WORKDIR/logs/gguf-print.out"
require "does not invent an Axolotl converter" "$WORKDIR/logs/gguf-print.out"
...
if [[ -e "$PREPARED/outputs/merged.gguf" || -e "$PREPARED/outputs/merged/model.gguf" ]]; then
  echo "FAIL  gguf-convert wrote a GGUF" >&2
  exit 1
fi

echo "-- empty GGUF is refuse:seat (magic required) --"
: > "$PREPARED/outputs/merged.gguf"
set +e
estate enrich local-seat \
  --prepared "$PREPARED" \
  --weights "$PREPARED/outputs/merged.gguf" \
...
rm -f "$PREPARED/outputs/merged.gguf"

# local-seat reads the first four bytes. An empty file is refuse:seat.
python3 - "$PREPARED/outputs/merged.gguf" <<'PY'
import pathlib, sys
...
require "ollama create ${TAG} -f ${PREPARED}/outputs/Modelfile" "$WORKDIR/logs/seat-gguf.out"
require "llama-cli -m " "$WORKDIR/logs/seat-gguf.out"
require "llama-server -m " "$WORKDIR/logs/seat-gguf.out"
require "estate enrich import-trained --estate <estate.yaml> --prepared ${PREPARED} --tag ${TAG} --adapter ${PREPARED}/outputs/merged.gguf" "$WORKDIR/logs/seat-gguf.out"
...
estate enrich import-trained \
  ...
  --prepared "$PREPARED" \
  --tag "$TAG" \
  --adapter "$PREPARED/outputs/merged.gguf" \
  --curator jason \
...
python3 - "$PREPARED/prepare.json" "$PREPARED/binding-proposal.json" "$PREPARED/outputs/merged.gguf" <<'PY'
import json, sys
...
grep -h -E '^(axolotl merge-lora |python3 convert_hf_to_gguf.py |ollama create |llama-cli -m |llama-server -m |estate enrich import-trained )' "$WORKDIR/logs/merge.out" "$WORKDIR/logs/gguf-print.out" "$WORKDIR/logs/seat-gguf.out"
echo
echo "PASS  axolotl-lora-journey (Axolotl LoRA card checked; merge without --dequant, convert, seat, and import printed; refuse:adapter; refuse:seat; refuse:tokenizer; SKIP live train; SKIP live convert; SKIP live seat)"
echo "READY_FOR_LIVE_TEST: no"
