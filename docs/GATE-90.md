...
| Feed source drivers | green | pack `source_drivers` is frontier and/or local and matches `path_counts`; INDEX and `estate feed list` print `drivers=` (`-` when empty); propose and accept copy the tag; promote stays off |
