...
The Day-90 factory gate is local cargo test. `make gate-90` wraps `cargo test --workspace`. Hosted CI is compile-only (`cargo check --workspace --locked` on `pull_request`). `READY_FOR_LIVE_TEST`: no. The live tip story is [`CELL-ONE-STATUS.md`](CELL-ONE-STATUS.md) through PR #151 (`ecbe8a1c9e5ebe80d581f2c82a2c194cf165aa67`): the qlora, lora, and seat journeys, the LLaMA-Factory beachhead matrix (PR #140), the print-only prepare walk (PR #142), the estate PATH / `ESTATE_BIN` fallback for journeys (PR #148), the tokenizer restore dereference (PR #149), the print-only local-seat Modelfile (PR #150), and help/docs that scope the write-Modelfile line to GGUF print-only seats (PR #151). Remaining names `make lf-beachhead-prepare`. See [`OPERATOR-DAY.md`](OPERATOR-DAY.md).
