...
| `make mlx-lm-lora-journey` | Opt-in print-only mlx-lm LoRA journey (operator section 16). Apple Silicon affinity only. Checks the `MLX.md` handoff and prints `mlx_lm.fuse` (`--export-gguf`), seat, and import against fixture stubs. `gguf-convert` stays `refuse:seat`. Does not train. Not native MLX. Not in smoke or Actions. Not a live train. |
| `make uniqueness-mlx` | Opt-in print-only chain of that journey: prepare-assert, then seat-print. Does not run `make unsloth-qlora-journey` or `make uniqueness-unsloth`. Does not train. Not in smoke or Actions. Not a live train. |
