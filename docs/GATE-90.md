...
The Day-90 factory gate is local cargo test. `make gate-90` wraps `cargo test --workspace`. Hosted CI is compile-only (`cargo check --workspace --locked` on `pull_request`). `READY_FOR_LIVE_TEST`: no. See git log for the current tip. This page does not invent a live PASS. The recorded 5090-class Target C live uniqueness PASS stays the only live uniqueness prove. Remaining names `make lf-beachhead-prepare`. See [`OPERATOR-DAY.md`](OPERATOR-DAY.md).
