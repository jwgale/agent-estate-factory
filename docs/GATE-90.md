...
The Day-90 factory gate is local cargo test. `make gate-90` wraps `cargo test --workspace`. Hosted CI is compile-only (`cargo check --workspace --locked` on `pull_request`). `READY_FOR_LIVE_TEST`: no. The live tip story is [`CELL-ONE-STATUS.md`](CELL-ONE-STATUS.md) through PR #143 (`3acdec3983ea581976649ba4b7cc41a4cd22d31d`): the qlora, lora, and seat journeys, the LLaMA-Factory beachhead matrix (PR #140), and the print-only prepare walk (PR #142). Remaining names `make lf-beachhead-prepare`. See [`OPERATOR-DAY.md`](OPERATOR-DAY.md).
