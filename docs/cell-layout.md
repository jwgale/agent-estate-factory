...
| `enrich/{pack_id}/{driver}/` | regenerable artifacts | `estate enrich prepare` default, including `--all-drivers`. Writes `prepare.json` (`cell-one.enrich-prepare.v0`), `PREPARE.md`, `NEXT.md`, and driver files (`Modelfile` whose `FROM` is the seated model, `manifest.json` + `manifest.yaml`, `recipe.yaml` + `export.yaml` + instruct chat `dataset.jsonl`, or `axolotl.yml` + `dataset.jsonl`). `estate enrich from-pack` uses this same tree. `estate enrich list` reads this tree and refuses when `enrich/` is missing. `estate enrich import-prepared` and `estate enrich import-trained` add `binding-proposal.json` and `binding-proposal.md` (`cell-one.enrich-binding-proposal.v0`) for the `local_slm` seat. Does not rewrite the estate. Does not train. Does not apply. Not copied by backup. `--out` can point at `packs/prepared/` instead. |
