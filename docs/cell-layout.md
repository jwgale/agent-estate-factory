...
| `enrich/{pack_id}/{driver}/` | regenerable artifacts | `estate enrich prepare` default, including `--all-drivers`. Writes `prepare.json` (`cell-one.enrich-prepare.v0`), `PREPARE.md`, `NEXT.md`, and driver files (`Modelfile` whose `FROM` is the seated model, `manifest.json` + `manifest.yaml`, `recipe.yaml` + `export.yaml` + instruct chat `dataset.jsonl` (`model_name_or_path` is the train base; `FROM` stays the seat tag), or `axolotl.yml` + `dataset.jsonl` on `axolotl-lora` (bf16 LoRA) and `axolotl-qlora` (4-bit QLoRA) (`base_model` is the train base; Modelfile `FROM` stays the seat tag; `dataset_mode` is `stub`, `scaffold`, or `feed` when `--from-feed` copied rows from `feed/`), or `UNSLOTH.md` on optional `unsloth-qlora` (operator-owned Nvidia-only handoff; no script), or `MLX.md` on optional `mlx-lm-lora` (operator-owned Apple Silicon LoRA handoff; written only when `host_class_affinity` is `apple-silicon`; no script)). `estate enrich from-pack` uses this same tree. `estate enrich list` reads this tree and refuses when `enrich/` is missing. `estate enrich import-prepared` and `estate enrich import-trained` add `binding-proposal.json` and `binding-proposal.md` (`cell-one.enrich-binding-proposal.v0`) for the `local_slm` seat. Does not rewrite the estate. Does not train. Does not apply. Not copied by backup. `--out` can point at `packs/prepared/` instead. |
| `enrich-stage/` | regenerable plan input | `estate enrich apply-proposal`. Writes `staged-estate.yaml` and `stage.json` (`cell-one.enrich-binding-stage.v0`). `estate plan` and `estate apply --require-plan` consume the staged estate. The source estate is copied only when that apply succeeds. `auto_apply` stays false. Not copied by backup. |
