...
An `agent` intention is who may call whom. Kind `agent` (aliases `agent_call`, `agent-call`). The object is an estate agent id, bare or `agent:`. The subject lists that id under `calls`. Missing coverage is deny-default. An explicit deny wins. Sacred ids are not call targets. Plan, drift, and doctor cite each edge as `own` (the same agent) or `peer` (another agent).
