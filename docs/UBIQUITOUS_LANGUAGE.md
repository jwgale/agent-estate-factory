...
The factory makes room for training and enrichment of purpose-built SLMs. The durable beachhead is `TrainEnrichDriver`, curator packs, and the specialist path (`estate specialist`, `local_slm`). Prepare writes artifacts. `llamafactory-lora` writes the unquantized LLaMA-Factory LoRA recipe an operator runs outside the factory, `llamafactory-qlora` writes the QLoRA recipe, `axolotl-lora` writes the bf16 Axolotl YAML, `axolotl-qlora` writes the 4-bit Axolotl YAML, and `import-trained` hands the adapter back through the existing proposal. Target C is the popular Qwen QLoRA ladder: that `llamafactory-qlora` prepare, the external train and export lines, a printed GGUF convert, a printed Ollama create, and `import-trained` to record the shape. Target A is the Qwen LoRA ladder: that `llamafactory-lora` prepare, the external train and export lines, a printed merge, a printed GGUF convert, a printed Ollama create, and `import-trained` to record the shape. `--from-feed` copies instruct rows already on disk. List reads them. Import-prepared proposes the `local_slm` join and leaves plan/apply to Jason. A dataset downloader and an in-process trainer stay unshipped. No new crate. Command surface: [`TRAIN-ENRICH.md`](TRAIN-ENRICH.md). Walks: [`operator-enrich-journeys.md`](operator-enrich-journeys.md).
