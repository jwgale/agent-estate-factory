...
Snapshot of **what is on `main` through PR #167**
...
-> [`operator-enrich-journeys.md`](operator-enrich-journeys.md) sections 8–14
...
Pages: [`TRAIN-ENRICH.md`](TRAIN-ENRICH.md). Walks: sections 8–14 of [`operator-enrich-journeys.md`](operator-enrich-journeys.md). Seat: [`local-seat.md`](local-seat.md). Live handoff: [`LIVE-PROBES.md`](LIVE-PROBES.md). `examples/estate.yaml` stays hash-locked. Prepare against that file is `refuse:base-model`.
...
| purpose-build checklist | The prepare → train → merge → seat → import loop had a re-prove card and no print-only card for purpose-build on demand. | This slice. `make purpose-build-checklist` prints that loop and points at the print journeys already on tip, then Standing next (estate). It does not execute them. It does not invent a live PASS. The recorded Target C PASS stays the only live uniqueness prove. The re-prove card stays `make uniqueness-prove-checklist`. Operator section 15. Not in smoke, `make gate-90`, or Actions. Tip header stays through PR #167. READY no. |
