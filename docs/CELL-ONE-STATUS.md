...
See git log for the current tip.
...
| operator surfaces name purpose-build | `docs/OPERATOR-DAY.md` and `docs/NORTH-STAR.md` did not name `make purpose-build-journey`, and GATE-90 Remaining for `make purpose-build-pick` omitted the DeepSeek-R1-Distill and GLM-4 Chat print targets the picker already prints. | `make purpose-build-journey` is the print-only purpose-build on-demand entry. It runs `make purpose-build-pick`, then `make purpose-build-checklist`. The pick Remaining row names DeepSeek-R1-Distill chat and GLM-4 Chat print targets and their uniqueness and LoRA twins. The picker does not run them. Not in smoke, `make gate-90`, or Actions. READY no. |
| mid-software-build purpose-build | Purpose-build mid-flow was named on the checklist section and not on the entry operators already use. | When an SLM fits mid-software-build, or on demand, the same print-only entry is `make purpose-build-journey`. It runs `make purpose-build-pick`, then `make purpose-build-checklist`. Those two targets are the parts. Not in smoke, `make gate-90`, or Actions. READY no. |
| help names mid-software-build | `estate help enrich`, `estate help train`, and the index Journey line named only the print-only purpose-build on-demand entry (operator section 18) and left mid-software-build off those surfaces. | When an SLM fits mid-software-build, or on demand, `make purpose-build-journey` is the same print-only purpose-build on-demand entry (operator section 18). It runs `make purpose-build-pick`, then `make purpose-build-checklist`. It does not inline those bodies. Not in smoke, `make gate-90`, or Actions. READY no. |
