...
Snapshot of **what is on `main` through PR #165**
...
Print-only `make unsloth-lora-journey` and `make uniqueness-unsloth-lora` are PR #165, operator section 14.
...
| unsloth doctor status | `estate doctor` and `estate status` did not lock `unsloth-qlora` and `unsloth-lora` as optional train cards. | Both commands print those cards as `status=optional` and `live=false`, report both prepare records, and do not claim a train or a promote. The catalog already says optional. Garbage or promoted `prepare.json` still fails closed. Operator sections 12 and 14 stay print-only. READY no. |
