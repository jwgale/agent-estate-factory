...
| train recipe | Prepare could label a job `train` and still had no trainer-facing recipe. | `axolotl-lora` writes `axolotl.yml` and `dataset.jsonl` plus the `axolotl train` line. It does not run Axolotl. `import-trained` records the adapter on the existing `local_slm` proposal. `make train-prepare` is opt-in and prints `SKIP live train`. READY no. |
