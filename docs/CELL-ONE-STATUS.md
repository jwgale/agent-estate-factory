...
| train recipe | The durable train recipe is LLaMA-Factory QLoRA. Axolotl stays the second YAML card. Unsloth is a NEXT.md pointer. | `llamafactory-qlora` writes `recipe.yaml`, `export.yaml`, and instruct chat `dataset.jsonl`. `axolotl-lora` stays the YAML card. Neither card runs a trainer. `import-trained` records the adapter on the existing `local_slm` proposal. `make train-prepare` prints `SKIP live train`. READY no. |
