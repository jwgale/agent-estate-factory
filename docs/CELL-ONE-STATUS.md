...
Snapshot of **what is on `main` through PR #175**
...
`estate help enrich` and `estate help train` name `make mlx-lm-lora-journey` as the print-only Apple Silicon mlx-lm LoRA journey (operator section 16) and `make uniqueness-mlx` as the print-only chain of that journey. The index help, the README opt-in one-liner, and the Makefile comments name that same path.
