...
| gauge and LoRA merge | A short factory gauge needed a hand-edit of `recipe.yaml`, and the merge step was easy to read as already finished. | `--max-steps`, `--cutoff-len`, `--lora-rank`, `--save-steps`, and `--gradient-accumulation-steps` write into `llamafactory-qlora`, `llamafactory-lora`, and `axolotl-lora`. `llamafactory-lora` omits quantization keys. `export.yaml` stays the merge card. `NEXT.md` points at `llamafactory-cli export` and says prepare did not merge. Invalid knobs refuse before write. READY no. |
