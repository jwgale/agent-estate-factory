...
| train recipe | The first train card was an Axolotl YAML recipe. A single Nvidia box is usually Unsloth QLoRA. | `unsloth-qlora` writes `train_unsloth.py` and instruct chat `dataset.jsonl`. `axolotl-lora` stays the YAML card. Neither card runs a trainer. `import-trained` records the adapter on the existing `local_slm` proposal. `make train-prepare` prints `SKIP live train`. READY no. |
