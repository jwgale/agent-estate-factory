...
the specialist path, and `estate enrich prepare` (artifacts only; `--all-drivers`, `from-pack`, seated Modelfile `FROM`, `llamafactory-qlora` writes a LLaMA-Factory QLoRA recipe and does not run it, `llamafactory-lora` writes the full-precision LoRA recipe and the merge card and does not merge, `axolotl-lora` writes an Axolotl recipe and does not run it, `estate enrich list`, `import-prepared` and `import-trained` as a `local_slm` proposal, and `apply-proposal` staging that proposal for `estate plan` / `estate apply --require-plan`).
