...
the specialist path, and `estate enrich prepare` (artifacts only; `--all-drivers`, `from-pack`, seated Modelfile `FROM`, `unsloth-qlora` writes a Unsloth QLoRA script and does not run it, `axolotl-lora` writes an Axolotl recipe and does not run it, `estate enrich list`, `import-prepared` and `import-trained` as a `local_slm` proposal, and `apply-proposal` staging that proposal for `estate plan` / `estate apply --require-plan`).
