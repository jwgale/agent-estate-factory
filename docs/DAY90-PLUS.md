...
the specialist path, and `estate enrich prepare` (artifacts only; `--all-drivers`, `from-pack`, seated Modelfile `FROM`, `estate enrich list`, `import-prepared` as a `local_slm` proposal, and `apply-proposal` staging that proposal for `estate plan` / `estate apply --require-plan`; no trainer).
