...
4. Seat with Ollama. On a GGUF whose Modelfile is not already on disk, `local-seat` is print-only. It prints the Modelfile and does not write `$PREPARED/Modelfile`. Write that file from the printed contents before `ollama create`. A GGUF also prints `llama-cli -m` and `llama-server -m` for that file. When `modelfile_on_disk=true`, the report uses the on-disk Modelfile when FROM already names the artifact. That is the merged directory when LLaMA-Factory wrote the Modelfile (`FROM .`), or a GGUF whose FROM already names that file. Do not write `$PREPARED/Modelfile` again. A merged directory is not a llama.cpp seat until the convert writes the sibling GGUF.
...
The 2026-09-23 Linux 5090-class run of this chain outside the factory is the recorded PASS [Target C live uniqueness (5090-class)](LIVE-PROBES.md). The factory did not train, convert, shell out to ollama, or promote. `make uniqueness-prove-checklist` prints those operator steps. It does not invent a new live PASS. After step 8 (`import-trained`, `trained_shape` `gguf`, `auto_apply=false`) it prints Standing next (estate). The proposal stays `auto_apply=false`. No promote and no auto-promote. It names `plan`, `apply --require-plan`, and `reconcile` and does not execute them.
...
`modelfile_on_disk=true` means that file is already the one to pass. The report uses the on-disk Modelfile when FROM already names the artifact. `FROM .` names the merged directory. Do not write `$PREPARED/Modelfile` again.

When `--weights` is a `.gguf` file and the Modelfile is not already on disk, `local-seat` is print-only. It prints the Modelfile and does not write `$PREPARED/Modelfile`. Write that file from the printed contents before `ollama create`. On the Target C ladder that file is the prepare directory's `Modelfile`, the `-f` argument of the create line (`<prepared>/export.gguf` sits beside it). When a LLaMA-Factory Modelfile is in the same directory and FROM does not already name the GGUF, TEMPLATE and PARAMETER lines are copied into the printed text. The file on disk is left as it was. When FROM already names that GGUF, the report uses the on-disk Modelfile when FROM already names the artifact. This factory does not invent a chat template.
...
`FROM` is `prepare.json` `seat_tag` (the same string as `base_model`). That Ollama model must already be the train base. `ADAPTER` is the adapter directory. The report then prints `ollama create cell-enrich-<pack-id> -f <directory>/Modelfile`. When that Modelfile is not already on disk, `local-seat` is print-only. It prints the Modelfile and does not write that file. Write that file from the printed contents before `ollama create`. When FROM is the seat tag and ADAPTER already names that directory, the report uses that on-disk Modelfile. The command does not run the line, does not merge, and does not promote.
