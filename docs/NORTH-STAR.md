...
5. **`make enrich-prepare`** — opt-in fixture. Example pack, both drivers, list, a binding proposal, then apply-proposal, plan, and require-plan apply on a throwaway lab copy. `examples/estate.yaml` stays hash-locked. Not a live train. Off smoke, `gate-90`, and Actions. Prepare: [`TRAIN-ENRICH.md`](TRAIN-ENRICH.md). Walks: [`operator-enrich-journeys.md`](operator-enrich-journeys.md).
