...
7. **`make train-prepare`** — opt-in Unsloth script and Axolotl recipe. Writes `train_unsloth.py` and `axolotl.yml` on a throwaway copy, checks `prepare.json` `job=train`, and leaves `examples/estate.yaml` alone. Prints `SKIP live train`. Does not run either trainer. Off smoke, `gate-90`, and Actions.
