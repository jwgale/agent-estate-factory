...
7. **`make train-prepare`** — opt-in Axolotl recipe. Writes `axolotl.yml` and `dataset.jsonl` on a throwaway copy, checks `prepare.json` `job=train`, and leaves `examples/estate.yaml` alone. Prints `SKIP live train`. Does not run Axolotl. Off smoke, `gate-90`, and Actions.
