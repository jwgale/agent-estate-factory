...
After a LLaMA-Factory export, or after `estate enrich merge-adapt` prints Axolotl's `axolotl merge-lora` line and Axolotl writes `output_dir/merged`, `estate enrich gguf-convert` prints the llama.cpp convert line for the merged directory, and `estate enrich local-seat` prints the `ollama create` line for that directory or a GGUF. A GGUF seat also prints `llama-cli -m` and `llama-server -m` for that file. `estate enrich local-seat --adapter` prints the no-merge seat for an adapter directory (`FROM` the seat tag, `ADAPTER` that directory). Axolotl does not write GGUF. These commands do not run Ollama, Axolotl, or llama.cpp. The Qwen QLoRA order that uses those prints is Target C in [`operator-enrich-journeys.md`](operator-enrich-journeys.md). The Qwen LoRA order is Target A in that same page. Page: [`local-seat.md`](local-seat.md).
