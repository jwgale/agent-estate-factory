...
5. `estate specialist --driver ollama --prompt "..."` - thin data-plane complete (same helper as `model-estate specialist --job complete`)
...
OpenAI-compatible / Ollama / llama.cpp server: the adapter posts the real request text to `/v1/chat/completions` or `/api/chat` after a models list answers. Policy jobs stay factory-owned. `complete` keeps the model text. Sacred refuse happens before any POST. A dummy ping is not a round-trip.
...
Jobs: `complete` (`estate specialist` default), `policy-precheck` (A8 default), or `redact`. Bound: fail closed over 16KiB. Do not echo secrets into the feed.
...
After a LLaMA-Factory export, or after `estate enrich merge-adapt` prints Axolotl's `axolotl merge-lora` line and Axolotl writes `output_dir/merged`, `estate enrich gguf-convert` prints the llama.cpp convert line for the merged directory, and `estate enrich local-seat` prints the `ollama create` line for that directory or a GGUF. A GGUF seat also prints `llama-cli -m` and `llama-server -m` for that file. `estate enrich local-seat --adapter` prints the no-merge seat for an adapter directory (`FROM` the seat tag, `ADAPTER` that directory). Axolotl does not write GGUF. These commands do not run Ollama, Axolotl, or llama.cpp. The Qwen QLoRA order that uses those prints is Target C in [`operator-enrich-journeys.md`](operator-enrich-journeys.md). The Qwen LoRA order is Target A in that same page. `make seat-journey` prints the Target C lines against fixture stubs (section 10). Page: [`local-seat.md`](local-seat.md).
