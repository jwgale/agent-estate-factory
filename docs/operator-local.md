...
After a LLaMA-Factory export, or after an operator merge of an Axolotl adapter into a Hugging Face directory, `estate enrich gguf-convert` prints the llama.cpp convert line for the merged directory, and `estate enrich local-seat` prints the `ollama create` line for that directory or a GGUF. `estate enrich local-seat --adapter` prints the no-merge seat for an adapter directory (`FROM` the seat tag, `ADAPTER` that directory). Axolotl does not write GGUF. Neither command runs Ollama or llama.cpp. The Qwen QLoRA order that uses those prints is Target C in [`operator-enrich-journeys.md`](operator-enrich-journeys.md). Page: [`local-seat.md`](local-seat.md).
