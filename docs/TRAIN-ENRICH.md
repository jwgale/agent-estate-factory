...
| `llamafactory-lora` | LoRA train card. Writes the same files as `llamafactory-qlora`: `recipe.yaml` is SFT LoRA (`finetuning_type: lora`, no `quantization_bit`, no `quantization_method`, LoRA rank 8, `cutoff_len` 512, `packing: false`). `template` comes from the train base (`qwen3_nothink` for a non-thinking Qwen3 instruct name such as `Qwen/Qwen3-4B-Instruct-2507`, `qwen3` for other Qwen3 names). `NEXT.md` and `PREPARE.md` name this card as the non-quant twin of the Qwen3 Instruct QLoRA prepare only when the winning segment is `Qwen/Qwen3-4B-Instruct-2507`, `Qwen/Qwen3-30B-A3B-Instruct-2507`, `Qwen/Qwen3-235B-A22B-Instruct-2507`, or `Qwen/Qwen3-Next-80B-A3B-Instruct`, matching `examples/train_lora/qwen3_lora_sft.yaml`. A thinking or Qwen2.5 id does not get that line. `NEXT.md` has `pip install llamafactory`, `llamafactory-cli train`, and `llamafactory-cli export`. This path does not require bitsandbytes. Does not shell out. |
