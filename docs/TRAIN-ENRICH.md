...
| `estate enrich prepare --all-drivers` | One call. Each card the job allows writes a sibling directory. A refuse writes none of them. The enrich default skips `llamafactory-lora`, `llamafactory-qlora`, `axolotl-lora`, `axolotl-qlora`, and `unsloth-qlora`. `--job train` includes them. |
...
| `make train-prepare` | Opt-in LLaMA-Factory LoRA and QLoRA recipes, Axolotl LoRA and QLoRA recipes, and the optional Unsloth handoff. Prints `SKIP live train`. Does not run a trainer. Not in `make smoke`, `make gate-90`, or GitHub Actions. |
...
Job field: `enrich` (default) or `train`. `llamafactory-lora`, `llamafactory-qlora`, `axolotl-lora`, `axolotl-qlora`, and `unsloth-qlora` default to `train` and refuse `enrich`. Both jobs only prepare. The factory does not run a trainer.
...
| `llamafactory-lora`, `llamafactory-qlora`, `axolotl-lora`, `axolotl-qlora`, or `unsloth-qlora` with `--job enrich` | `refuse:job` |
...
| `import-trained` on a prepare that is not `llamafactory-lora`, `llamafactory-qlora`, `axolotl-lora`, `axolotl-qlora`, or `unsloth-qlora` with job `train` | `refuse:driver` or `refuse:job` |
...
| `llamafactory-lora`, `llamafactory-qlora`, `axolotl-lora`, `axolotl-qlora`, or `unsloth-qlora` has no train base, or the value is a bare Ollama tag or a seat-looking local leaf, or `model_name_or_path` / `axolotl.yml` `base_model` / `UNSLOTH.md` `train_base_model` does not match `prepare.json` `train_base_model` | `refuse:train-base` |
