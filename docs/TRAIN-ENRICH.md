...
`make qlora-journey` leaves `gguf-convert` and `local-seat` at `refuse:seat` because the merged export and the GGUF are not there yet. `make seat-journey` is the opt-in that prints the happy path once fixture stubs stand in for those files. Walk: section 10 of [`operator-enrich-journeys.md`](operator-enrich-journeys.md). `estate help enrich` names the same check.
...
| `gguf-convert` `tokenizer_config.json` has `extra_special_tokens` as a JSON list (or another non-object), is a symlink, is not JSON, or a Qwen-family export is missing `vocab.json` or `merges.txt` | `refuse:tokenizer` (copy tokenizer files from the train base already on disk; keep the export `tokenizer_config.json` as `tokenizer_config.json.bak`; this factory does not download or copy them) |
