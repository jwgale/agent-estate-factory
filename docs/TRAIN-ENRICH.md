...
`axolotl-qlora` is 4-bit QLoRA. It matches Axolotl `examples/llama-3/qlora.yml`. `load_in_8bit` is false, `load_in_4bit` is true, and `adapter` is `qlora`. `sequence_len` is 4096, `micro_batch_size` is 2, `gradient_accumulation_steps` is 4, and `lora_r` is 32, matching that file. `lora_alpha` is 16, `lora_dropout` is 0.05, `num_epochs` is 4, `optimizer` is `paged_adamw_32bit`, `learning_rate` is 0.0002, and `warmup_ratio` is 0.1, matching that file. `sample_packing` is true and `lora_target_linear` is true, matching that file. `val_set_size` is 0.0, matching that file's 0. `evals_per_epoch` is 0. That file sets 4, which asks for eval on an empty split. That file sets `flash_attention: true` and a Llama pad token. This card leaves both unset.
