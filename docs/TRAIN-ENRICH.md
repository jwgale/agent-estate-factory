...
| `merge-adapt` prepare is not `llamafactory-lora`, `llamafactory-qlora`, `axolotl-lora`, or `axolotl-qlora` with job `train`, or `--adapter` is missing `adapter_config.json`, is a merged export, is a GGUF, is a symlink, or the Axolotl yaml disagrees with the card | `refuse:driver`, `refuse:job`, `refuse:adapter`, or `refuse:merge` |
