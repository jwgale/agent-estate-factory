...
`train_base_model` is an additive optional field on `pack.v0` and `specialist-pack.v0`. Empty means unset. `llamafactory-qlora` also reads `params.train_base_model` on the local binding when the pack field is empty. A bare Ollama seat tag in that field is `refuse:train-base`.
