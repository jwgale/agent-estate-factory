...
`train-enrich.v0.json` is the prepare envelope (`cell-one.enrich-prepare.v0`). `promoted`, `auto_apply`, and `estate_rewritten` stay false. `ollama-modelfile` adds a Modelfile. `external-manifest` adds `manifest.json` and `manifest.yaml` (`base_model`, `purpose`, `host_class_affinity`, `dataset_paths`). `NEXT.md` is the operator handoff. A new required field on this envelope is a v1.

`enrich-binding-proposal.v0.json` is the `local_slm` join (`cell-one.enrich-binding-proposal.v0`). `estate enrich import-prepared` writes it next to `prepare.json`. `auto_apply`, `promoted`, and `estate_rewritten` stay false. Paste the snippet, then `estate plan` and `estate apply`. The command does not apply.
