...
`train-enrich.v0.json` is the prepare envelope (`cell-one.enrich-prepare.v0`). `promoted`, `auto_apply`, and `estate_rewritten` stay false. `ollama-modelfile` adds a Modelfile. `FROM` and `base_model` are the seated model (`params.model` or a model-tag hint), not the binding id `local_slm`. `external-manifest` adds `manifest.json` and `manifest.yaml` (`base_model`, `purpose`, `host_class_affinity`, `dataset_paths`). `axolotl-lora` adds `axolotl.yml` and `dataset.jsonl` for an Axolotl train the operator runs outside the factory. `NEXT.md` is the operator handoff. A new required field on this envelope is a v1.
